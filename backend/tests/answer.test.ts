...
describe("answerQuestion", () => {
  ...
  it("supports Japanese numeral runner-up wording", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の二位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
  ...
  it("clarifies an underspecified aragyoku split-ranking query", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の区間賞は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
    expect(result.kind).toBe("offline");
    if (result.kind === "offline") {
      expect(result.text).toContain("区間賞・区間順位は、年度・性別・区間を指定してください");
      expect(result.text).not.toContain("区間賞（区間1位）");
    }
  });
  ...
  it("supports Japanese numeral fourth-place wording", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の四位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
  ...
  it("answers a yearless gendered aragyoku third-place question", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝女子の3位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
  ...
  it("summarizes yearless gendered aragyoku runner-up questions", async () => {
    resetRetrieverCache();
    resetKgCache();
    for (const [question, expected] of [
      ["荒玉駅伝男子の2位は？", "2025年男子準優勝校: 玉陵（58:02）"],
      ["荒玉駅伝女子の準優勝タイムは？", "2025年女子準優勝校: 南関（43:45）"],
    ]) {
  ...
  it("labels both genders in an unqualified nagomi winner question", async () => {
    resetRetrieverCache();
    resetKgCache();
    ...
    expect(result.kind).toBe("offline");
    if (result.kind === "offline") {
      expect(result.text).toContain("2026年なごみ男子優勝: NJAC（38:33）");
      expect(result.text).toContain("2026年なごみ女子優勝: 金栗PROJECT A（26:55）");
    }
  });
  ...
  it("answers an unqualified sixth-place question for both genders", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の6位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
  ...
  it("answers unqualified seventh-to-ninth-place questions for both genders", async () => {
    ...
    resetKgCache();
    const expected = [
      ["荒玉駅伝の7位は？", "2025年男子7位: 南関（59:18）", "2025年女子7位: 岱明（45:22）"],
      ["荒玉駅伝の8位は？", "2025年男子8位: 玉東（59:45）", "2025年女子8位: 荒尾海陽（46:16）"],
      ["荒玉駅伝の9位は？", "2025年男子9位: 玉名（60:52）", "2025年女子9位: 菊水（46:40）"],
    ];
  ...
  it("supports Japanese numeral eighth-place wording", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の八位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
    expect(result.kind).toBe("offline");
    if (result.kind === "offline") {
      expect(result.text).toContain("2025年男子8位: 玉東（59:45）");
      expect(result.text).toContain("2025年女子8位: 荒尾海陽（46:16）");
      expect(result.text).not.toContain("深掘り分析");
    }
  });
  ...
  it("supports Japanese numeral twelfth-place wording", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の十二位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
    expect(result.kind).toBe("offline");
    if (result.kind === "offline") {
      expect(result.text).toContain("2025年男子12位: 有明（62:31）");
      expect(result.text).toContain("2025年女子12位: 有明（47:24）");
      expect(result.text).not.toContain("深掘り分析");
    }
  });

  it("supports Japanese numeral thirteenth-place wording", async () => {
    resetRetrieverCache();
    resetKgCache();
    const result = await answerQuestion("荒玉駅伝の十三位は？", {
      llm: null,
      skipRouter: true,
      defaultYear: 2026,
    });
    ...
    if (result.kind === "offline") {
      expect(result.text).toContain("2025年男子13位: 玉南（62:41）");
      expect(result.text).toContain("2025年女子13位: 三加和（48:29）");
      expect(result.text).not.toContain("深掘り分析");
    }
  });
  ...
  it("answers unqualified thirteenth-to-fifteenth-place questions", async () => {
    resetRetrieverCache();
    resetKgCache();
    const expected = [
      ["荒玉駅伝の13位は？", "2025年男子13位: 玉南（62:41）", "2025年女子13位: 三加和（48:29）"],
      ["荒玉駅伝の14位は？", "2025年男子14位: 天水（63:39）", "2025年女子14位: 玉陵（49:36）"],
      ["荒玉駅伝の15位は？", "2025年男子15位: 三加和（—）", "2025年女子15位: 天水（51:29）"],
    ];
    for (const [question, male, female] of expected) {
      const result = await answerQuestion(question, { llm: null, skipRouter: true, defaultYear: 2026 });
      expect(result.kind).toBe("offline");
