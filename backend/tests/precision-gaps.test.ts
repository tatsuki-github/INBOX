...
describe("QA precision regressions", () => {
  ...
  it("answers 玉名's year and gender-specific total from the matching team row", async () => {
    const result = await ask("2025女子優勝の玉名の総合タイムは？");
    expect(result.sources).toEqual(["out-analysis/aragyoku-teams/玉名.md"]);
    expect(result.text).toContain("2025年荒玉駅伝女子 玉名は1位・総合41:58");
    expect(result.text).not.toContain("60:52");
  });

  it("answers a winner's year and gender-specific time from the matching team row", async () => {
    const result = await ask("2024年荒玉男子の優勝は南関でタイムは？");
    expect(result.sources).toEqual(["out-analysis/aragyoku-teams/南関.md"]);
    expect(result.text).toContain("2024年荒玉駅伝男子 南関は1位・総合56:38");
  ...
  it("returns the historical year for a named team's runner-up finish", async () => {
    ...
    expect(result.text).toContain("2024年");
  ...
  it("uses the 2026 ranking when it has a newer named 3000m SB", async () => {
    const result = await ask("松浦眞大の3000mSBは？");
    expect(result.sources).toEqual([
      "out-analysis/2026_aragyoku_men_3000m_sb_ranking.md",
    ]);
    expect(result.text).toContain("松浦眞大");
    expect(result.text).toContain("9:08.80");
    expect(result.text).not.toContain("9:32.58");
  });

  it("extracts a named athlete from beyond the top rows of the 3000m ranking", async () => {
    const result = await ask("松野凛空の3000mSBは？");
    expect(result.sources).toEqual([
      "out-analysis/2026_aragyoku_men_3000m_sb_ranking.md",
    ]);
    expect(result.text).toContain("9:37.84");
  ...
  it("answers the date and venue for the named 玉名市合同練習会", async () => {
    ...
    expect(result.sources).toEqual([
      "drive-text/練習/玉名市練習会/2026-09-22.md",
    ]);
    expect(result.text).toContain("2026年9月22日");
    expect(result.text).toContain("おおはまふれあいセンター");
  });

  it("keeps なごみ駅伝 lineup questions out of the generic 荒玉 clarification", async () => {
    const result = await ask("なごみ駅伝の岱明男子1区は誰？");
    expect(result.sources).toEqual([
  ...
  it.each([
    "南関中の稗島葵音の800mから5kmまでのSBは？",
    "南関中 稗島葵音の1500mから5kmまでのSBは？",
    "稗島葵音（南関中）の1500mから5kmまでのSBは？",
    "稗島葵音さん（南関中）の1500m〜5000mSBは？",
    "稗島葵音の1500m〜5000mSB（南関中）",
    "稗島葵音（南関中）1500m～5kmSB",
    "南関中 稗島葵音 1500m～5km SB",
    "南関中の稗島葵音の800メートルから5kmまでSB",
    "稗島葵音（南関中）の800mから5000mまでのSB",
    "稗島葵音さん 南関中 800mから5kmのSBは？",
  ])("extracts the athlete name around school context in a range query: %s", async (question) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["sb/中学生SB.csv"]);
    expect(result.text).toContain("稗島葵音（南関中）");
    expect(result.text).toContain("5000m 11:18.46");
  ...
  it.each([
    ["女子選手たち1000mの結果", "村上"], ["男子選手たち1000mの結果", "松野"],
    ["女子選手たち1000mのタイム", "増岡"], ["男子選手たち1000mのタイム", "田上"],
    ["女子選手たち1000mの参加者", "山﨑"], ["男子選手たち1000mの参加者", "山本"],
    ["9月22日の女子選手たち1000mの結果", "角田"], ["9月22日の男子選手たち1000mの結果", "中尾"],
    ["女子選手たち1000mはどうだった？", "塚原"], ["男子選手たち1000mはどうだった？", "南本"],
  ])("routes athletes-plural wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子選手陣1000mの結果", "村上"], ["男子選手陣1000mの結果", "松野"],
    ["女子選手陣1000mのタイム", "増岡"], ["男子選手陣1000mのタイム", "田上"],
    ["女子選手陣1000mの参加者", "山﨑"], ["男子選手陣1000mの参加者", "山本"],
    ["9月22日の女子選手陣1000mの結果", "角田"], ["9月22日の男子選手陣1000mの結果", "中尾"],
    ["女子選手陣1000mはどうだった？", "塚原"], ["男子選手陣1000mはどうだった？", "南本"],
  ])("routes athlete-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子メンバーたち1000mの結果", "村上"], ["男子メンバーたち1000mの結果", "松野"],
    ["女子メンバーたち1000mのタイム", "増岡"], ["男子メンバーたち1000mのタイム", "田上"],
    ["女子メンバーたち1000mの参加者", "山﨑"], ["男子メンバーたち1000mの参加者", "山本"],
    ["9月22日の女子メンバーたち1000mの結果", "角田"], ["9月22日の男子メンバーたち1000mの結果", "中尾"],
    ["女子メンバーたち1000mはどうだった？", "塚原"], ["男子メンバーたち1000mはどうだった？", "南本"],
  ])("routes members-plural wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子参加者たち1000mの結果", "村上"], ["男子参加者たち1000mの結果", "松野"],
    ["女子参加者たち1000mのタイム", "増岡"], ["男子参加者たち1000mのタイム", "田上"],
    ["女子参加者たち1000mの参加者", "山﨑"], ["男子参加者たち1000mの参加者", "山本"],
    ["9月22日の女子参加者たち1000mの結果", "角田"], ["9月22日の男子参加者たち1000mの結果", "中尾"],
    ["女子参加者たち1000mはどうだった？", "塚原"], ["男子参加者たち1000mはどうだった？", "南本"],
  ])("routes participants-plural wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子出場者1000mの結果", "村上"], ["男子出場者1000mの結果", "松野"],
    ["女子出場者1000mのタイム", "増岡"], ["男子出場者1000mのタイム", "田上"],
    ["女子出場者1000mの参加者", "山﨑"], ["男子出場者1000mの参加者", "山本"],
    ["9月22日の女子出場者1000mの結果", "角田"], ["9月22日の男子出場者1000mの結果", "中尾"],
    ["女子出場者1000mはどうだった？", "塚原"], ["男子出場者1000mはどうだった？", "南本"],
  ])("routes entrants-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子走者たち1000mの結果", "村上"], ["男子走者たち1000mの結果", "松野"],
    ["女子走者たち1000mのタイム", "増岡"], ["男子走者たち1000mのタイム", "田上"],
    ["女子走者たち1000mの参加者", "山﨑"], ["男子走者たち1000mの参加者", "山本"],
    ["9月22日の女子走者たち1000mの結果", "角田"], ["9月22日の男子走者たち1000mの結果", "中尾"],
    ["女子走者たち1000mはどうだった？", "塚原"], ["男子走者たち1000mはどうだった？", "南本"],
  ])("routes runners-plural wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子ランナーたち1000mの結果", "村上"], ["男子ランナーたち1000mの結果", "松野"],
    ["女子ランナーたち1000mのタイム", "増岡"], ["男子ランナーたち1000mのタイム", "田上"],
    ["女子ランナーたち1000mの参加者", "山﨑"], ["男子ランナーたち1000mの参加者", "山本"],
    ["9月22日の女子ランナーたち1000mの結果", "角田"], ["9月22日の男子ランナーたち1000mの結果", "中尾"],
    ["女子ランナーたち1000mはどうだった？", "塚原"], ["男子ランナーたち1000mはどうだった？", "南本"],
  ])("routes runners-term-plural wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子種目の1000mの結果", "村上"], ["男子種目の1000mの結果", "松野"],
    ["女子種目の1000mのタイム", "増岡"], ["男子種目の1000mのタイム", "田上"],
    ["女子種目の1000mの参加者", "山﨑"], ["男子種目の1000mの参加者", "山本"],
    ["9月22日の女子種目の1000mの結果", "角田"], ["9月22日の男子種目の1000mの結果", "中尾"],
    ["女子種目の1000mはどうだった？", "塚原"], ["男子種目の1000mはどうだった？", "南本"],
  ])("routes event-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子競技の1000mの結果", "村上"], ["男子競技の1000mの結果", "松野"],
    ["女子競技の1000mのタイム", "増岡"], ["男子競技の1000mのタイム", "田上"],
    ["女子競技の1000mの参加者", "山﨑"], ["男子競技の1000mの参加者", "山本"],
    ["9月22日の女子競技の1000mの結果", "角田"], ["9月22日の男子競技の1000mの結果", "中尾"],
    ["女子競技の1000mはどうだった？", "塚原"], ["男子競技の1000mはどうだった？", "南本"],
  ])("routes competition-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子種目としての1000mの結果", "村上"], ["男子種目としての1000mの結果", "松野"],
    ["女子種目としての1000mのタイム", "増岡"], ["男子種目としての1000mのタイム", "田上"],
    ["女子種目としての1000mの参加者", "山﨑"], ["男子種目としての1000mの参加者", "山本"],
    ["9月22日の女子種目としての1000mの結果", "角田"], ["9月22日の男子種目としての1000mの結果", "中尾"],
    ["女子種目としての1000mはどうだった？", "塚原"], ["男子種目としての1000mはどうだった？", "南本"],
  ])("routes event-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子レースの1000mの結果", "村上"], ["男子レースの1000mの結果", "松野"],
    ["女子レースの1000mのタイム", "増岡"], ["男子レースの1000mのタイム", "田上"],
    ["女子レースの1000mの参加者", "山﨑"], ["男子レースの1000mの参加者", "山本"],
    ["9月22日の女子レースの1000mの結果", "角田"], ["9月22日の男子レースの1000mの結果", "中尾"],
    ["女子レースの1000mはどうだった？", "塚原"], ["男子レースの1000mはどうだった？", "南本"],
  ])("routes race-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子記録会の1000mの結果", "村上"], ["男子記録会の1000mの結果", "松野"],
    ["女子記録会の1000mのタイム", "増岡"], ["男子記録会の1000mのタイム", "田上"],
    ["女子記録会の1000mの参加者", "山﨑"], ["男子記録会の1000mの参加者", "山本"],
    ["9月22日の女子記録会の1000mの結果", "角田"], ["9月22日の男子記録会の1000mの結果", "中尾"],
    ["女子記録会の1000mはどうだった？", "塚原"], ["男子記録会の1000mはどうだった？", "南本"],
  ])("routes record-meet-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子大会の1000mの結果", "村上"], ["男子大会の1000mの結果", "松野"],
    ["女子大会の1000mのタイム", "増岡"], ["男子大会の1000mのタイム", "田上"],
    ["女子大会の1000mの参加者", "山﨑"], ["男子大会の1000mの参加者", "山本"],
    ["9月22日の女子大会の1000mの結果", "角田"], ["9月22日の男子大会の1000mの結果", "中尾"],
    ["女子大会の1000mはどうだった？", "塚原"], ["男子大会の1000mはどうだった？", "南本"],
  ])("routes meet-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子練習の1000mの結果", "村上"], ["男子練習の1000mの結果", "松野"],
    ["女子練習の1000mのタイム", "増岡"], ["男子練習の1000mのタイム", "田上"],
    ["女子練習の1000mの参加者", "山﨑"], ["男子練習の1000mの参加者", "山本"],
    ["9月22日の女子練習の1000mの結果", "角田"], ["9月22日の男子練習の1000mの結果", "中尾"],
    ["女子練習の1000mはどうだった？", "塚原"], ["男子練習の1000mはどうだった？", "南本"],
  ])("routes practice-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子走の1000mの結果", "村上"], ["男子走の1000mの結果", "松野"],
    ["女子走の1000mのタイム", "増岡"], ["男子走の1000mのタイム", "田上"],
    ["女子走の1000mの参加者", "山﨑"], ["男子走の1000mの参加者", "山本"],
    ["9月22日の女子走の1000mの結果", "角田"], ["9月22日の男子走の1000mの結果", "中尾"],
    ["女子走の1000mはどうだった？", "塚原"], ["男子走の1000mはどうだった？", "南本"],
  ])("routes run-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子記録としての1000mの結果", "村上"], ["男子記録としての1000mの結果", "松野"],
    ["女子記録としての1000mのタイム", "増岡"], ["男子記録としての1000mのタイム", "田上"],
    ["女子記録としての1000mの参加者", "山﨑"], ["男子記録としての1000mの参加者", "山本"],
    ["9月22日の女子記録としての1000mの結果", "角田"], ["9月22日の男子記録としての1000mの結果", "中尾"],
    ["女子記録としての1000mはどうだった？", "塚原"], ["男子記録としての1000mはどうだった？", "南本"],
  ])("routes record-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子結果としての1000mの結果", "村上"], ["男子結果としての1000mの結果", "松野"],
    ["女子結果としての1000mのタイム", "増岡"], ["男子結果としての1000mのタイム", "田上"],
    ["女子結果としての1000mの参加者", "山﨑"], ["男子結果としての1000mの参加者", "山本"],
    ["9月22日の女子結果としての1000mの結果", "角田"], ["9月22日の男子結果としての1000mの結果", "中尾"],
    ["女子結果としての1000mはどうだった？", "塚原"], ["男子結果としての1000mはどうだった？", "南本"],
  ])("routes result-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子による1000mの結果", "村上"], ["男子による1000mの結果", "松野"],
    ["女子による1000mのタイム", "増岡"], ["男子による1000mのタイム", "田上"],
    ["女子による1000mの参加者", "山﨑"], ["男子による1000mの参加者", "山本"],
    ["9月22日の女子による1000mの結果", "角田"], ["9月22日の男子による1000mの結果", "中尾"],
    ["女子による1000mはどうだった？", "塚原"], ["男子による1000mはどうだった？", "南本"],
  ])("routes by-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が走った1000mの結果", "村上"], ["男子が走った1000mの結果", "松野"],
    ["女子が走った1000mのタイム", "増岡"], ["男子が走った1000mのタイム", "田上"],
    ["女子が走った1000mの参加者", "山﨑"], ["男子が走った1000mの参加者", "山本"],
    ["9月22日の女子が走った1000mの結果", "角田"], ["9月22日の男子が走った1000mの結果", "中尾"],
    ["女子が走った1000mはどうだった？", "塚原"], ["男子が走った1000mはどうだった？", "南本"],
  ])("routes past-run wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が出た1000mの結果", "村上"], ["男子が出た1000mの結果", "松野"],
    ["女子が出た1000mのタイム", "増岡"], ["男子が出た1000mのタイム", "田上"],
    ["女子が出た1000mの参加者", "山﨑"], ["男子が出た1000mの参加者", "山本"],
    ["9月22日の女子が出た1000mの結果", "角田"], ["9月22日の男子が出た1000mの結果", "中尾"],
    ["女子が出た1000mはどうだった？", "塚原"], ["男子が出た1000mはどうだった？", "南本"],
  ])("routes participation-past wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が参加した1000mの結果", "村上"], ["男子が参加した1000mの結果", "松野"],
    ["女子が参加した1000mのタイム", "増岡"], ["男子が参加した1000mのタイム", "田上"],
    ["女子が参加した1000mの参加者", "山﨑"], ["男子が参加した1000mの参加者", "山本"],
    ["9月22日の女子が参加した1000mの結果", "角田"], ["9月22日の男子が参加した1000mの結果", "中尾"],
    ["女子が参加した1000mはどうだった？", "塚原"], ["男子が参加した1000mはどうだった？", "南本"],
  ])("routes participation-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が出場した1000mの結果", "村上"], ["男子が出場した1000mの結果", "松野"],
    ["女子が出場した1000mのタイム", "増岡"], ["男子が出場した1000mのタイム", "田上"],
    ["女子が出場した1000mの参加者", "山﨑"], ["男子が出場した1000mの参加者", "山本"],
    ["9月22日の女子が出場した1000mの結果", "角田"], ["9月22日の男子が出場した1000mの結果", "中尾"],
    ["女子が出場した1000mはどうだった？", "塚原"], ["男子が出場した1000mはどうだった？", "南本"],
  ])("routes attendance-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が走る1000mの結果", "村上"], ["男子が走る1000mの結果", "松野"],
    ["女子が走る1000mのタイム", "増岡"], ["男子が走る1000mのタイム", "田上"],
    ["女子が走る1000mの参加者", "山﨑"], ["男子が走る1000mの参加者", "山本"],
    ["9月22日の女子が走る1000mの結果", "角田"], ["9月22日の男子が走る1000mの結果", "中尾"],
    ["女子が走る1000mはどうだった？", "塚原"], ["男子が走る1000mはどうだった？", "南本"],
  ])("routes run-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を対象にした1000mの結果", "村上"], ["男子を対象にした1000mの結果", "松野"],
    ["女子を対象にした1000mのタイム", "増岡"], ["男子を対象にした1000mのタイム", "田上"],
    ["女子を対象にした1000mの参加者", "山﨑"], ["男子を対象にした1000mの参加者", "山本"],
    ["9月22日の女子を対象にした1000mの結果", "角田"], ["9月22日の男子を対象にした1000mの結果", "中尾"],
    ["女子を対象にした1000mはどうだった？", "塚原"], ["男子を対象にした1000mはどうだった？", "南本"],
  ])("routes targeted-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を含む1000mの結果", "村上"], ["男子を含む1000mの結果", "松野"],
    ["女子を含む1000mのタイム", "増岡"], ["男子を含む1000mのタイム", "田上"],
    ["女子を含む1000mの参加者", "山﨑"], ["男子を含む1000mの参加者", "山本"],
    ["9月22日の女子を含む1000mの結果", "角田"], ["9月22日の男子を含む1000mの結果", "中尾"],
    ["女子を含む1000mはどうだった？", "塚原"], ["男子を含む1000mはどうだった？", "南本"],
  ])("routes include-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のみの1000mの結果", "村上"], ["男子のみの1000mの結果", "松野"],
    ["女子のみの1000mのタイム", "増岡"], ["男子のみの1000mのタイム", "田上"],
    ["女子のみの1000mの参加者", "山﨑"], ["男子のみの1000mの参加者", "山本"],
    ["9月22日の女子のみの1000mの結果", "角田"], ["9月22日の男子のみの1000mの結果", "中尾"],
    ["女子のみの1000mはどうだった？", "塚原"], ["男子のみの1000mはどうだった？", "南本"],
  ])("routes exclusive-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子限定の1000mの結果", "村上"], ["男子限定の1000mの結果", "松野"],
    ["女子限定の1000mのタイム", "増岡"], ["男子限定の1000mのタイム", "田上"],
    ["女子限定の1000mの参加者", "山﨑"], ["男子限定の1000mの参加者", "山本"],
    ["9月22日の女子限定の1000mの結果", "角田"], ["9月22日の男子限定の1000mの結果", "中尾"],
    ["女子限定の1000mはどうだった？", "塚原"], ["男子限定の1000mはどうだった？", "南本"],
  ])("routes limited-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のみ参加の1000mの結果", "村上"], ["男子のみ参加の1000mの結果", "松野"],
    ["女子のみ参加の1000mのタイム", "増岡"], ["男子のみ参加の1000mのタイム", "田上"],
    ["女子のみ参加の1000mの参加者", "山﨑"], ["男子のみ参加の1000mの参加者", "山本"],
    ["9月22日の女子のみ参加の1000mの結果", "角田"], ["9月22日の男子のみ参加の1000mの結果", "中尾"],
    ["女子のみ参加の1000mはどうだった？", "塚原"], ["男子のみ参加の1000mはどうだった？", "南本"],
  ])("routes participation-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子だけの1000mの結果", "村上"], ["男子だけの1000mの結果", "松野"],
    ["女子だけの1000mのタイム", "増岡"], ["男子だけの1000mのタイム", "田上"],
    ["女子だけの1000mの参加者", "山﨑"], ["男子だけの1000mの参加者", "山本"],
    ["9月22日の女子だけの1000mの結果", "角田"], ["9月22日の男子だけの1000mの結果", "中尾"],
    ["女子だけの1000mはどうだった？", "塚原"], ["男子だけの1000mはどうだった？", "南本"],
  ])("routes only-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子中心となる1000mの結果", "村上"], ["男子中心となる1000mの結果", "松野"],
    ["女子中心となる1000mのタイム", "増岡"], ["男子中心となる1000mのタイム", "田上"],
    ["女子中心となる1000mの参加者", "山﨑"], ["男子中心となる1000mの参加者", "山本"],
    ["9月22日の女子中心となる1000mの結果", "角田"], ["9月22日の男子中心となる1000mの結果", "中尾"],
    ["女子中心となる1000mはどうだった？", "塚原"], ["男子中心となる1000mはどうだった？", "南本"],
  ])("routes center-result wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子主体の1000mの結果", "村上"], ["男子主体の1000mの結果", "松野"],
    ["女子主体の1000mのタイム", "増岡"], ["男子主体の1000mのタイム", "田上"],
    ["女子主体の1000mの参加者", "山﨑"], ["男子主体の1000mの参加者", "山本"],
    ["9月22日の女子主体の1000mの結果", "角田"], ["9月22日の男子主体の1000mの結果", "中尾"],
    ["女子主体の1000mはどうだった？", "塚原"], ["男子主体の1000mはどうだった？", "南本"],
  ])("routes subject-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子対象となる1000mの結果", "村上"], ["男子対象となる1000mの結果", "松野"],
    ["女子対象となる1000mのタイム", "増岡"], ["男子対象となる1000mのタイム", "田上"],
    ["女子対象となる1000mの参加者", "山﨑"], ["男子対象となる1000mの参加者", "山本"],
    ["9月22日の女子対象となる1000mの結果", "角田"], ["9月22日の男子対象となる1000mの結果", "中尾"],
    ["女子対象となる1000mはどうだった？", "塚原"], ["男子対象となる1000mはどうだった？", "南本"],
  ])("routes target-result wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を中心とした1000mの結果", "村上"], ["男子を中心とした1000mの結果", "松野"],
    ["女子を中心とした1000mのタイム", "増岡"], ["男子を中心とした1000mのタイム", "田上"],
    ["女子を中心とした1000mの参加者", "山﨑"], ["男子を中心とした1000mの参加者", "山本"],
    ["9月22日の女子を中心とした1000mの結果", "角田"], ["9月22日の男子を中心とした1000mの結果", "中尾"],
    ["女子を中心とした1000mはどうだった？", "塚原"], ["男子を中心とした1000mはどうだった？", "南本"],
  ])("routes centered-on wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を対象とした1000mの結果", "村上"], ["男子を対象とした1000mの結果", "松野"],
    ["女子を対象とした1000mのタイム", "増岡"], ["男子を対象とした1000mのタイム", "田上"],
    ["女子を対象とした1000mの参加者", "山﨑"], ["男子を対象とした1000mの参加者", "山本"],
    ["9月22日の女子を対象とした1000mの結果", "角田"], ["9月22日の男子を対象とした1000mの結果", "中尾"],
    ["女子を対象とした1000mはどうだった？", "塚原"], ["男子を対象とした1000mはどうだった？", "南本"],
  ])("routes targeted-on wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を中心にした1000mの結果", "村上"], ["男子を中心にした1000mの結果", "松野"],
    ["女子を中心にした1000mのタイム", "増岡"], ["男子を中心にした1000mのタイム", "田上"],
    ["女子を中心にした1000mの参加者", "山﨑"], ["男子を中心にした1000mの参加者", "山本"],
    ["9月22日の女子を中心にした1000mの結果", "角田"], ["9月22日の男子を中心にした1000mの結果", "中尾"],
    ["女子を中心にした1000mはどうだった？", "塚原"], ["男子を中心にした1000mはどうだった？", "南本"],
  ])("routes centered-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を主とする1000mの結果", "村上"], ["男子を主とする1000mの結果", "松野"],
    ["女子を主とする1000mのタイム", "増岡"], ["男子を主とする1000mのタイム", "田上"],
    ["女子を主とする1000mの参加者", "山﨑"], ["男子を主とする1000mの参加者", "山本"],
    ["9月22日の女子を主とする1000mの結果", "角田"], ["9月22日の男子を主とする1000mの結果", "中尾"],
    ["女子を主とする1000mはどうだった？", "塚原"], ["男子を主とする1000mはどうだった？", "南本"],
  ])("routes principal-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に関する1000mの結果", "村上"], ["男子に関する1000mの結果", "松野"],
    ["女子に関する1000mのタイム", "増岡"], ["男子に関する1000mのタイム", "田上"],
    ["女子に関する1000mの参加者", "山﨑"], ["男子に関する1000mの参加者", "山本"],
    ["9月22日の女子に関する1000mの結果", "角田"], ["9月22日の男子に関する1000mの結果", "中尾"],
    ["女子に関する1000mはどうだった？", "塚原"], ["男子に関する1000mはどうだった？", "南本"],
  ])("routes related-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子をめぐる1000mの結果", "村上"], ["男子をめぐる1000mの結果", "松野"],
    ["女子をめぐる1000mのタイム", "増岡"], ["男子をめぐる1000mのタイム", "田上"],
    ["女子をめぐる1000mの参加者", "山﨑"], ["男子をめぐる1000mの参加者", "山本"],
    ["9月22日の女子をめぐる1000mの結果", "角田"], ["9月22日の男子をめぐる1000mの結果", "中尾"],
    ["女子をめぐる1000mはどうだった？", "塚原"], ["男子をめぐる1000mはどうだった？", "南本"],
  ])("routes around-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子をめぐって1000mの結果", "村上"], ["男子をめぐって1000mの結果", "松野"],
    ["女子をめぐって1000mのタイム", "増岡"], ["男子をめぐって1000mのタイム", "田上"],
    ["女子をめぐって1000mの参加者", "山﨑"], ["男子をめぐって1000mの参加者", "山本"],
    ["9月22日の女子をめぐって1000mの結果", "角田"], ["9月22日の男子をめぐって1000mの結果", "中尾"],
    ["女子をめぐって1000mはどうだった？", "塚原"], ["男子をめぐって1000mはどうだった？", "南本"],
  ])("routes around-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子における1000mの結果", "村上"], ["男子における1000mの結果", "松野"],
    ["女子における1000mのタイム", "増岡"], ["男子における1000mのタイム", "田上"],
    ["女子における1000mの参加者", "山﨑"], ["男子における1000mの参加者", "山本"],
    ["9月22日の女子における1000mの結果", "角田"], ["9月22日の男子における1000mの結果", "中尾"],
    ["女子における1000mはどうだった？", "塚原"], ["男子における1000mはどうだった？", "南本"],
  ])("routes in-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に対する1000mの結果", "村上"], ["男子に対する1000mの結果", "松野"],
    ["女子に対する1000mのタイム", "増岡"], ["男子に対する1000mのタイム", "田上"],
    ["女子に対する1000mの参加者", "山﨑"], ["男子に対する1000mの参加者", "山本"],
    ["9月22日の女子に対する1000mの結果", "角田"], ["9月22日の男子に対する1000mの結果", "中尾"],
    ["女子に対する1000mはどうだった？", "塚原"], ["男子に対する1000mはどうだった？", "南本"],
  ])("routes against-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に関して1000mの結果", "村上"], ["男子に関して1000mの結果", "松野"],
    ["女子に関して1000mのタイム", "増岡"], ["男子に関して1000mのタイム", "田上"],
    ["女子に関して1000mの参加者", "山﨑"], ["男子に関して1000mの参加者", "山本"],
    ["9月22日の女子に関して1000mの結果", "角田"], ["9月22日の男子に関して1000mの結果", "中尾"],
    ["女子に関して1000mはどうだった？", "塚原"], ["男子に関して1000mはどうだった？", "南本"],
  ])("routes regarding-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に該当する1000mの結果", "村上"], ["男子に該当する1000mの結果", "松野"],
    ["女子に該当する1000mのタイム", "増岡"], ["男子に該当する1000mのタイム", "田上"],
    ["女子に該当する1000mの参加者", "山﨑"], ["男子に該当する1000mの参加者", "山本"],
    ["9月22日の女子に該当する1000mの結果", "角田"], ["9月22日の男子に該当する1000mの結果", "中尾"],
    ["女子に該当する1000mはどうだった？", "塚原"], ["男子に該当する1000mはどうだった？", "南本"],
  ])("routes matching-verb wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に当たる1000mの結果", "村上"], ["男子に当たる1000mの結果", "松野"],
    ["女子に当たる1000mのタイム", "増岡"], ["男子に当たる1000mのタイム", "田上"],
    ["女子に当たる1000mの参加者", "山﨑"], ["男子に当たる1000mの参加者", "山本"],
    ["9月22日の女子に当たる1000mの結果", "角田"], ["9月22日の男子に当たる1000mの結果", "中尾"],
    ["女子に当たる1000mはどうだった？", "塚原"], ["男子に当たる1000mはどうだった？", "南本"],
  ])("routes applicable-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子向けとしての1000mの結果", "村上"], ["男子向けとしての1000mの結果", "松野"],
    ["女子向けとしての1000mのタイム", "増岡"], ["男子向けとしての1000mのタイム", "田上"],
    ["女子向けとしての1000mの参加者", "山﨑"], ["男子向けとしての1000mの参加者", "山本"],
    ["9月22日の女子向けとしての1000mの結果", "角田"], ["9月22日の男子向けとしての1000mの結果", "中尾"],
    ["女子向けとしての1000mはどうだった？", "塚原"], ["男子向けとしての1000mはどうだった？", "南本"],
  ])("routes audience-context wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を想定した1000mの結果", "村上"], ["男子を想定した1000mの結果", "松野"],
    ["女子を想定した1000mのタイム", "増岡"], ["男子を想定した1000mのタイム", "田上"],
    ["女子を想定した1000mの参加者", "山﨑"], ["男子を想定した1000mの参加者", "山本"],
    ["9月22日の女子を想定した1000mの結果", "角田"], ["9月22日の男子を想定した1000mの結果", "中尾"],
    ["女子を想定した1000mはどうだった？", "塚原"], ["男子を想定した1000mはどうだった？", "南本"],
  ])("routes assumed-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を意識した1000mの結果", "村上"], ["男子を意識した1000mの結果", "松野"],
    ["女子を意識した1000mのタイム", "増岡"], ["男子を意識した1000mのタイム", "田上"],
    ["女子を意識した1000mの参加者", "山﨑"], ["男子を意識した1000mの参加者", "山本"],
    ["9月22日の女子を意識した1000mの結果", "角田"], ["9月22日の男子を意識した1000mの結果", "中尾"],
    ["女子を意識した1000mはどうだった？", "塚原"], ["男子を意識した1000mはどうだった？", "南本"],
  ])("routes gender-aware wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に関わる1000mの結果", "村上"], ["男子に関わる1000mの結果", "松野"],
    ["女子に関わる1000mのタイム", "増岡"], ["男子に関わる1000mのタイム", "田上"],
    ["女子に関わる1000mの参加者", "山﨑"], ["男子に関わる1000mの参加者", "山本"],
    ["9月22日の女子に関わる1000mの結果", "角田"], ["9月22日の男子に関わる1000mの結果", "中尾"],
    ["女子に関わる1000mはどうだった？", "塚原"], ["男子に関わる1000mはどうだった？", "南本"],
  ])("routes involved-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子にまつわる1000mの結果", "村上"], ["男子にまつわる1000mの結果", "松野"],
    ["女子にまつわる1000mのタイム", "増岡"], ["男子にまつわる1000mのタイム", "田上"],
    ["女子にまつわる1000mの参加者", "山﨑"], ["男子にまつわる1000mの参加者", "山本"],
    ["9月22日の女子にまつわる1000mの結果", "角田"], ["9月22日の男子にまつわる1000mの結果", "中尾"],
    ["女子にまつわる1000mはどうだった？", "塚原"], ["男子にまつわる1000mはどうだった？", "南本"],
  ])("routes associated-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子をめぐる結果の1000mの結果", "村上"], ["男子をめぐる結果の1000mの結果", "松野"],
    ["女子をめぐる結果の1000mのタイム", "増岡"], ["男子をめぐる結果の1000mのタイム", "田上"],
    ["女子をめぐる結果の1000mの参加者", "山﨑"], ["男子をめぐる結果の1000mの参加者", "山本"],
    ["9月22日の女子をめぐる結果の1000mの結果", "角田"], ["9月22日の男子をめぐる結果の1000mの結果", "中尾"],
    ["女子をめぐる結果の1000mはどうだった？", "塚原"], ["男子をめぐる結果の1000mはどうだった？", "南本"],
  ])("routes result-around wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の参加する1000mの結果", "村上"], ["男子の参加する1000mの結果", "松野"],
    ["女子の参加する1000mのタイム", "増岡"], ["男子の参加する1000mのタイム", "田上"],
    ["女子の参加する1000mの参加者", "山﨑"], ["男子の参加する1000mの参加者", "山本"],
    ["9月22日の女子の参加する1000mの結果", "角田"], ["9月22日の男子の参加する1000mの結果", "中尾"],
    ["女子の参加する1000mはどうだった？", "塚原"], ["男子の参加する1000mはどうだった？", "南本"],
  ])("routes participating-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の出場する1000mの結果", "村上"], ["男子の出場する1000mの結果", "松野"],
    ["女子の出場する1000mのタイム", "増岡"], ["男子の出場する1000mのタイム", "田上"],
    ["女子の出場する1000mの参加者", "山﨑"], ["男子の出場する1000mの参加者", "山本"],
    ["9月22日の女子の出場する1000mの結果", "角田"], ["9月22日の男子の出場する1000mの結果", "中尾"],
    ["女子の出場する1000mはどうだった？", "塚原"], ["男子の出場する1000mはどうだった？", "南本"],
  ])("routes attending-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が対象の1000mの結果", "村上"], ["男子が対象の1000mの結果", "松野"],
    ["女子が対象の1000mのタイム", "増岡"], ["男子が対象の1000mのタイム", "田上"],
    ["女子が対象の1000mの参加者", "山﨑"], ["男子が対象の1000mの参加者", "山本"],
    ["9月22日の女子が対象の1000mの結果", "角田"], ["9月22日の男子が対象の1000mの結果", "中尾"],
    ["女子が対象の1000mはどうだった？", "塚原"], ["男子が対象の1000mはどうだった？", "南本"],
  ])("routes subject-target wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が中心の1000mの結果", "村上"], ["男子が中心の1000mの結果", "松野"],
    ["女子が中心の1000mのタイム", "増岡"], ["男子が中心の1000mのタイム", "田上"],
    ["女子が中心の1000mの参加者", "山﨑"], ["男子が中心の1000mの参加者", "山本"],
    ["9月22日の女子が中心の1000mの結果", "角田"], ["9月22日の男子が中心の1000mの結果", "中尾"],
    ["女子が中心の1000mはどうだった？", "塚原"], ["男子が中心の1000mはどうだった？", "南本"],
  ])("routes subject-center wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子が主体の1000mの結果", "村上"], ["男子が主体の1000mの結果", "松野"],
    ["女子が主体の1000mのタイム", "増岡"], ["男子が主体の1000mのタイム", "田上"],
    ["女子が主体の1000mの参加者", "山﨑"], ["男子が主体の1000mの参加者", "山本"],
    ["9月22日の女子が主体の1000mの結果", "角田"], ["9月22日の男子が主体の1000mの結果", "中尾"],
    ["女子が主体の1000mはどうだった？", "塚原"], ["男子が主体の1000mはどうだった？", "南本"],
  ])("routes subject-main wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を主役にした1000mの結果", "村上"], ["男子を主役にした1000mの結果", "松野"],
    ["女子を主役にした1000mのタイム", "増岡"], ["男子を主役にした1000mのタイム", "田上"],
    ["女子を主役にした1000mの参加者", "山﨑"], ["男子を主役にした1000mの参加者", "山本"],
    ["9月22日の女子を主役にした1000mの結果", "角田"], ["9月22日の男子を主役にした1000mの結果", "中尾"],
    ["女子を主役にした1000mはどうだった？", "塚原"], ["男子を主役にした1000mはどうだった？", "南本"],
  ])("routes main-role wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に焦点を当てた1000mの結果", "村上"], ["男子に焦点を当てた1000mの結果", "松野"],
    ["女子に焦点を当てた1000mのタイム", "増岡"], ["男子に焦点を当てた1000mのタイム", "田上"],
    ["女子に焦点を当てた1000mの参加者", "山﨑"], ["男子に焦点を当てた1000mの参加者", "山本"],
    ["9月22日の女子に焦点を当てた1000mの結果", "角田"], ["9月22日の男子に焦点を当てた1000mの結果", "中尾"],
    ["女子に焦点を当てた1000mはどうだった？", "塚原"], ["男子に焦点を当てた1000mはどうだった？", "南本"],
  ])("routes focus-target wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子に着目した1000mの結果", "村上"], ["男子に着目した1000mの結果", "松野"],
    ["女子に着目した1000mのタイム", "増岡"], ["男子に着目した1000mのタイム", "田上"],
    ["女子に着目した1000mの参加者", "山﨑"], ["男子に着目した1000mの参加者", "山本"],
    ["9月22日の女子に着目した1000mの結果", "角田"], ["9月22日の男子に着目した1000mの結果", "中尾"],
    ["女子に着目した1000mはどうだった？", "塚原"], ["男子に着目した1000mはどうだった？", "南本"],
  ])("routes attention-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を取り上げた1000mの結果", "村上"], ["男子を取り上げた1000mの結果", "松野"],
    ["女子を取り上げた1000mのタイム", "増岡"], ["男子を取り上げた1000mのタイム", "田上"],
    ["女子を取り上げた1000mの参加者", "山﨑"], ["男子を取り上げた1000mの参加者", "山本"],
    ["9月22日の女子を取り上げた1000mの結果", "角田"], ["9月22日の男子を取り上げた1000mの結果", "中尾"],
    ["女子を取り上げた1000mはどうだった？", "塚原"], ["男子を取り上げた1000mはどうだった？", "南本"],
  ])("routes featured-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を比較した1000mの結果", "村上"], ["男子を比較した1000mの結果", "松野"],
    ["女子を比較した1000mのタイム", "増岡"], ["男子を比較した1000mのタイム", "田上"],
    ["女子を比較した1000mの参加者", "山﨑"], ["男子を比較した1000mの参加者", "山本"],
    ["9月22日の女子を比較した1000mの結果", "角田"], ["9月22日の男子を比較した1000mの結果", "中尾"],
    ["女子を比較した1000mはどうだった？", "塚原"], ["男子を比較した1000mはどうだった？", "南本"],
  ])("routes comparison-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を含めた1000mの結果", "村上"], ["男子を含めた1000mの結果", "松野"],
    ["女子を含めた1000mのタイム", "増岡"], ["男子を含めた1000mのタイム", "田上"],
    ["女子を含めた1000mの参加者", "山﨑"], ["男子を含めた1000mの参加者", "山本"],
    ["9月22日の女子を含めた1000mの結果", "角田"], ["9月22日の男子を含めた1000mの結果", "中尾"],
    ["女子を含めた1000mはどうだった？", "塚原"], ["男子を含めた1000mはどうだった？", "南本"],
  ])("routes included-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を分けた1000mの結果", "村上"], ["男子を分けた1000mの結果", "松野"],
    ["女子を分けた1000mのタイム", "増岡"], ["男子を分けた1000mのタイム", "田上"],
    ["女子を分けた1000mの参加者", "山﨑"], ["男子を分けた1000mの参加者", "山本"],
    ["9月22日の女子を分けた1000mの結果", "角田"], ["9月22日の男子を分けた1000mの結果", "中尾"],
    ["女子を分けた1000mはどうだった？", "塚原"], ["男子を分けた1000mはどうだった？", "南本"],
  ])("routes split-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子を分けて1000mの結果", "村上"], ["男子を分けて1000mの結果", "松野"],
    ["女子を分けて1000mのタイム", "増岡"], ["男子を分けて1000mのタイム", "田上"],
    ["女子を分けて1000mの参加者", "山﨑"], ["男子を分けて1000mの参加者", "山本"],
    ["9月22日の女子を分けて1000mの結果", "角田"], ["9月22日の男子を分けて1000mの結果", "中尾"],
    ["女子を分けて1000mはどうだった？", "塚原"], ["男子を分けて1000mはどうだった？", "南本"],
  ])("routes split-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子別に見た1000mの結果", "村上"], ["男子別に見た1000mの結果", "松野"],
    ["女子別に見た1000mのタイム", "増岡"], ["男子別に見た1000mのタイム", "田上"],
    ["女子別に見た1000mの参加者", "山﨑"], ["男子別に見た1000mの参加者", "山本"],
    ["9月22日の女子別に見た1000mの結果", "角田"], ["9月22日の男子別に見た1000mの結果", "中尾"],
    ["女子別に見た1000mはどうだった？", "塚原"], ["男子別に見た1000mはどうだった？", "南本"],
  ])("routes viewed-separately wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子ごとの1000mの結果", "村上"], ["男子ごとの1000mの結果", "松野"],
    ["女子ごとの1000mのタイム", "増岡"], ["男子ごとの1000mのタイム", "田上"],
    ["女子ごとの1000mの参加者", "山﨑"], ["男子ごとの1000mの参加者", "山本"],
    ["9月22日の女子ごとの1000mの結果", "角田"], ["9月22日の男子ごとの1000mの結果", "中尾"],
    ["女子ごとの1000mはどうだった？", "塚原"], ["男子ごとの1000mはどうだった？", "南本"],
  ])("routes each-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子ごとに1000mの結果", "村上"], ["男子ごとに1000mの結果", "松野"],
    ["女子ごとに1000mのタイム", "増岡"], ["男子ごとに1000mのタイム", "田上"],
    ["女子ごとに1000mの参加者", "山﨑"], ["男子ごとに1000mの参加者", "山本"],
    ["9月22日の女子ごとに1000mの結果", "角田"], ["9月22日の男子ごとに1000mの結果", "中尾"],
    ["女子ごとに1000mはどうだった？", "塚原"], ["男子ごとに1000mはどうだった？", "南本"],
  ])("routes per-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子単独の1000mの結果", "村上"], ["男子単独の1000mの結果", "松野"],
    ["女子単独の1000mのタイム", "増岡"], ["男子単独の1000mのタイム", "田上"],
    ["女子単独の1000mの参加者", "山﨑"], ["男子単独の1000mの参加者", "山本"],
    ["9月22日の女子単独の1000mの結果", "角田"], ["9月22日の男子単独の1000mの結果", "中尾"],
    ["女子単独の1000mはどうだった？", "塚原"], ["男子単独の1000mはどうだった？", "南本"],
  ])("routes standalone-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ...
    ["9月22日の女子選手一同1000mの結果", "角田"], ["9月22日の男子選手一同1000mの結果", "中尾"],
    ["女子選手一同1000mはどうだった？", "塚原"], ["男子選手一同1000mはどうだった？", "南本"],
  ])("routes athletes-collective wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女の子1000mの結果", "村上"], ["男の子1000mの結果", "松野"],
    ["女の子1000mのタイム", "増岡"], ["男の子1000mのタイム", "田上"],
    ["女の子1000mの参加者", "山﨑"], ["男の子1000mの参加者", "山本"],
    ["9月22日の女の子1000mの結果", "角田"], ["9月22日の男の子1000mの結果", "中尾"],
    ["女の子1000mはどうだった？", "塚原"], ["男の子1000mはどうだった？", "南本"],
  ])("routes colloquial-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["おんなの子1000mの結果", "村上"], ["おとこの子1000mの結果", "松野"],
    ["おんなの子1000mのタイム", "増岡"], ["おとこの子1000mのタイム", "田上"],
    ["おんなの子1000mの参加者", "山﨑"], ["おとこの子1000mの参加者", "山本"],
    ["9月22日のおんなの子1000mの結果", "角田"], ["9月22日のおとこの子1000mの結果", "中尾"],
    ["おんなの子1000mはどうだった？", "塚原"], ["おとこの子1000mはどうだった？", "南本"],
  ])("routes hiragana-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子クラス1000mの結果", "村上"], ["男子クラス1000mの結果", "松野"],
    ["女子クラス1000mのタイム", "増岡"], ["男子クラス1000mのタイム", "田上"],
    ["女子クラス1000mの参加者", "山﨑"], ["男子クラス1000mの参加者", "山本"],
    ["9月22日の女子クラス1000mの結果", "角田"], ["9月22日の男子クラス1000mの結果", "中尾"],
    ["女子クラス1000mはどうだった？", "塚原"], ["男子クラス1000mはどうだった？", "南本"],
  ])("routes class-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子学年1000mの結果", "村上"], ["男子学年1000mの結果", "松野"],
    ["女子学年1000mのタイム", "増岡"], ["男子学年1000mのタイム", "田上"],
    ["女子学年1000mの参加者", "山﨑"], ["男子学年1000mの参加者", "山本"],
    ["9月22日の女子学年1000mの結果", "角田"], ["9月22日の男子学年1000mの結果", "中尾"],
    ["女子学年1000mはどうだった？", "塚原"], ["男子学年1000mはどうだった？", "南本"],
  ])("routes grade-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子選抜1000mの結果", "村上"], ["男子選抜1000mの結果", "松野"],
    ["女子選抜1000mのタイム", "増岡"], ["男子選抜1000mのタイム", "田上"],
    ["女子選抜1000mの参加者", "山﨑"], ["男子選抜1000mの参加者", "山本"],
    ["9月22日の女子選抜1000mの結果", "角田"], ["9月22日の男子選抜1000mの結果", "中尾"],
    ["女子選抜1000mはどうだった？", "塚原"], ["男子選抜1000mはどうだった？", "南本"],
  ])("routes selection-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子候補1000mの結果", "村上"], ["男子候補1000mの結果", "松野"],
    ["女子候補1000mのタイム", "増岡"], ["男子候補1000mのタイム", "田上"],
    ["女子候補1000mの参加者", "山﨑"], ["男子候補1000mの参加者", "山本"],
    ["9月22日の女子候補1000mの結果", "角田"], ["9月22日の男子候補1000mの結果", "中尾"],
    ["女子候補1000mはどうだった？", "塚原"], ["男子候補1000mはどうだった？", "南本"],
  ])("routes candidate-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子登録1000mの結果", "村上"], ["男子登録1000mの結果", "松野"],
    ["女子登録1000mのタイム", "増岡"], ["男子登録1000mのタイム", "田上"],
    ["女子登録1000mの参加者", "山﨑"], ["男子登録1000mの参加者", "山本"],
    ["9月22日の女子登録1000mの結果", "角田"], ["9月22日の男子登録1000mの結果", "中尾"],
    ["女子登録1000mはどうだった？", "塚原"], ["男子登録1000mはどうだった？", "南本"],
  ])("routes registration-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子所属1000mの結果", "村上"], ["男子所属1000mの結果", "松野"],
    ["女子所属1000mのタイム", "増岡"], ["男子所属1000mのタイム", "田上"],
    ["女子所属1000mの参加者", "山﨑"], ["男子所属1000mの参加者", "山本"],
    ["9月22日の女子所属1000mの結果", "角田"], ["9月22日の男子所属1000mの結果", "中尾"],
    ["女子所属1000mはどうだった？", "塚原"], ["男子所属1000mはどうだった？", "南本"],
  ])("routes affiliation-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子チーム1000mの結果", "村上"], ["男子チーム1000mの結果", "松野"],
    ["女子チーム1000mのタイム", "増岡"], ["男子チーム1000mのタイム", "田上"],
    ["女子チーム1000mの参加者", "山﨑"], ["男子チーム1000mの参加者", "山本"],
    ["9月22日の女子チーム1000mの結果", "角田"], ["9月22日の男子チーム1000mの結果", "中尾"],
    ["女子チーム1000mはどうだった？", "塚原"], ["男子チーム1000mはどうだった？", "南本"],
  ])("routes team-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子大会1000mの結果", "村上"], ["男子大会1000mの結果", "松野"],
    ["女子大会1000mのタイム", "増岡"], ["男子大会1000mのタイム", "田上"],
    ["女子大会1000mの参加者", "山﨑"], ["男子大会1000mの参加者", "山本"],
    ["9月22日の女子大会1000mの結果", "角田"], ["9月22日の男子大会1000mの結果", "中尾"],
    ["女子大会1000mはどうだった？", "塚原"], ["男子大会1000mはどうだった？", "南本"],
  ])("routes meet-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子種目別1000mの結果", "村上"], ["男子種目別1000mの結果", "松野"],
    ["女子種目別1000mのタイム", "増岡"], ["男子種目別1000mのタイム", "田上"],
    ["女子種目別1000mの参加者", "山﨑"], ["男子種目別1000mの参加者", "山本"],
    ["9月22日の女子種目別1000mの結果", "角田"], ["9月22日の男子種目別1000mの結果", "中尾"],
    ["女子種目別1000mはどうだった？", "塚原"], ["男子種目別1000mはどうだった？", "南本"],
  ])("routes event-type-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子レース1000mの結果", "村上"], ["男子レース1000mの結果", "松野"],
    ["女子レース1000mのタイム", "増岡"], ["男子レース1000mのタイム", "田上"],
    ["女子レース1000mの参加者", "山﨑"], ["男子レース1000mの参加者", "山本"],
    ["9月22日の女子レース1000mの結果", "角田"], ["9月22日の男子レース1000mの結果", "中尾"],
    ["女子レース1000mはどうだった？", "塚原"], ["男子レース1000mはどうだった？", "南本"],
  ])("routes race-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子組1000mの結果", "村上"], ["男子組1000mの結果", "松野"],
    ["女子組1000mのタイム", "増岡"], ["男子組1000mのタイム", "田上"],
    ["女子組1000mの参加者", "山﨑"], ["男子組1000mの参加者", "山本"],
    ["9月22日の女子組1000mの結果", "角田"], ["9月22日の男子組1000mの結果", "中尾"],
    ["女子組1000mはどうだった？", "塚原"], ["男子組1000mはどうだった？", "南本"],
  ])("routes heat-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子部1000mの結果", "村上"], ["男子部1000mの結果", "松野"],
    ["女子部1000mのタイム", "増岡"], ["男子部1000mのタイム", "田上"],
    ["女子部1000mの参加者", "山﨑"], ["男子部1000mの参加者", "山本"],
    ["9月22日の女子部1000mの結果", "角田"], ["9月22日の男子部1000mの結果", "中尾"],
    ["女子部1000mはどうだった？", "塚原"], ["男子部1000mはどうだった？", "南本"],
  ])("routes division-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  ...
  it.each([
    ["女子カテゴリ別1000mの結果", "村上"], ["男子カテゴリ別1000mの結果", "松野"],
    ["女子カテゴリ別1000mのタイム", "増岡"], ["男子カテゴリ別1000mのタイム", "田上"],
    ["女子カテゴリ別1000mの参加者", "山﨑"], ["男子カテゴリ別1000mの参加者", "山本"],
    ["9月22日の女子カテゴリ別1000mの結果", "角田"], ["9月22日の男子カテゴリ別1000mの結果", "中尾"],
    ["女子カテゴリ別1000mはどうだった？", "塚原"], ["男子カテゴリ別1000mはどうだった？", "南本"],
  ])("routes category-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子区分別1000mの結果", "村上"], ["男子区分別1000mの結果", "松野"],
    ["女子区分別1000mのタイム", "増岡"], ["男子区分別1000mのタイム", "田上"],
    ["女子区分別1000mの参加者", "山﨑"], ["男子区分別1000mの参加者", "山本"],
    ["9月22日の女子区分別1000mの結果", "角田"], ["9月22日の男子区分別1000mの結果", "中尾"],
    ["女子区分別1000mはどうだった？", "塚原"], ["男子区分別1000mはどうだった？", "南本"],
  ])("routes segment-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子クラス別1000mの結果", "村上"], ["男子クラス別1000mの結果", "松野"],
    ["女子クラス別1000mのタイム", "増岡"], ["男子クラス別1000mのタイム", "田上"],
    ["女子クラス別1000mの参加者", "山﨑"], ["男子クラス別1000mの参加者", "山本"],
    ["9月22日の女子クラス別1000mの結果", "角田"], ["9月22日の男子クラス別1000mの結果", "中尾"],
    ["女子クラス別1000mはどうだった？", "塚原"], ["男子クラス別1000mはどうだった？", "南本"],
  ])("routes class-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子選手層別1000mの結果", "村上"], ["男子選手層別1000mの結果", "松野"],
    ["女子選手層別1000mのタイム", "増岡"], ["男子選手層別1000mのタイム", "田上"],
    ["女子選手層別1000mの参加者", "山﨑"], ["男子選手層別1000mの参加者", "山本"],
    ["9月22日の女子選手層別1000mの結果", "角田"], ["9月22日の男子選手層別1000mの結果", "中尾"],
    ["女子選手層別1000mはどうだった？", "塚原"], ["男子選手層別1000mはどうだった？", "南本"],
  ])("routes athlete-tier wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子ランキング1000mの結果", "村上"], ["男子ランキング1000mの結果", "松野"],
    ["女子ランキング1000mのタイム", "増岡"], ["男子ランキング1000mのタイム", "田上"],
    ["女子ランキング1000mの参加者", "山﨑"], ["男子ランキング1000mの参加者", "山本"],
    ["9月22日の女子ランキング1000mの結果", "角田"], ["9月22日の男子ランキング1000mの結果", "中尾"],
    ["女子ランキング1000mはどうだった？", "塚原"], ["男子ランキング1000mはどうだった？", "南本"],
  ])("routes ranking-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子上位1000mの結果", "村上"], ["男子上位1000mの結果", "松野"],
    ["女子上位1000mのタイム", "増岡"], ["男子上位1000mのタイム", "田上"],
    ["女子上位1000mの参加者", "山﨑"], ["男子上位1000mの参加者", "山本"],
    ["9月22日の女子上位1000mの結果", "角田"], ["9月22日の男子上位1000mの結果", "中尾"],
    ["女子上位1000mはどうだった？", "塚原"], ["男子上位1000mはどうだった？", "南本"],
  ])("routes top-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子先頭1000mの結果", "村上"], ["男子先頭1000mの結果", "松野"],
    ["女子先頭1000mのタイム", "増岡"], ["男子先頭1000mのタイム", "田上"],
    ["女子先頭1000mの参加者", "山﨑"], ["男子先頭1000mの参加者", "山本"],
    ["9月22日の女子先頭1000mの結果", "角田"], ["9月22日の男子先頭1000mの結果", "中尾"],
    ["女子先頭1000mはどうだった？", "塚原"], ["男子先頭1000mはどうだった？", "南本"],
  ])("routes front-runner wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子出走1000mの結果", "村上"], ["男子出走1000mの結果", "松野"],
    ["女子出走1000mのタイム", "増岡"], ["男子出走1000mのタイム", "田上"],
    ["女子出走1000mの参加者", "山﨑"], ["男子出走1000mの参加者", "山本"],
    ["9月22日の女子出走1000mの結果", "角田"], ["9月22日の男子出走1000mの結果", "中尾"],
    ["女子出走1000mはどうだった？", "塚原"], ["男子出走1000mはどうだった？", "南本"],
  ])("routes start-list wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子参加1000mの結果", "村上"], ["男子参加1000mの結果", "松野"],
    ["女子参加1000mのタイム", "増岡"], ["男子参加1000mのタイム", "田上"],
    ["女子参加1000mの参加者", "山﨑"], ["男子参加1000mの参加者", "山本"],
    ["9月22日の女子参加1000mの結果", "角田"], ["9月22日の男子参加1000mの結果", "中尾"],
    ["女子参加1000mはどうだった？", "塚原"], ["男子参加1000mはどうだった？", "南本"],
  ])("routes participation-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子出場1000mの結果", "村上"], ["男子出場1000mの結果", "松野"],
    ["女子出場1000mのタイム", "増岡"], ["男子出場1000mのタイム", "田上"],
    ["女子出場1000mの参加者", "山﨑"], ["男子出場1000mの参加者", "山本"],
    ["9月22日の女子出場1000mの結果", "角田"], ["9月22日の男子出場1000mの結果", "中尾"],
    ["女子出場1000mはどうだった？", "塚原"], ["男子出場1000mはどうだった？", "南本"],
  ])("routes attendance-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子記録会1000mの結果", "村上"], ["男子記録会1000mの結果", "松野"],
    ["女子記録会1000mのタイム", "増岡"], ["男子記録会1000mのタイム", "田上"],
    ["女子記録会1000mの参加者", "山﨑"], ["男子記録会1000mの参加者", "山本"],
    ["9月22日の女子記録会1000mの結果", "角田"], ["9月22日の男子記録会1000mの結果", "中尾"],
    ["女子記録会1000mはどうだった？", "塚原"], ["男子記録会1000mはどうだった？", "南本"],
  ])("routes record-meet wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子タイム1000mの結果", "村上"], ["男子タイム1000mの結果", "松野"],
    ["女子タイム1000mのタイム", "増岡"], ["男子タイム1000mのタイム", "田上"],
    ["女子タイム1000mの参加者", "山﨑"], ["男子タイム1000mの参加者", "山本"],
    ["9月22日の女子タイム1000mの結果", "角田"], ["9月22日の男子タイム1000mの結果", "中尾"],
    ["女子タイム1000mはどうだった？", "塚原"], ["男子タイム1000mはどうだった？", "南本"],
  ])("routes time-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子結果1000mの結果", "村上"], ["男子結果1000mの結果", "松野"],
    ["女子結果1000mのタイム", "増岡"], ["男子結果1000mのタイム", "田上"],
    ["女子結果1000mの参加者", "山﨑"], ["男子結果1000mの参加者", "山本"],
    ["9月22日の女子結果1000mの結果", "角田"], ["9月22日の男子結果1000mの結果", "中尾"],
    ["女子結果1000mはどうだった？", "塚原"], ["男子結果1000mはどうだった？", "南本"],
  ])("routes result-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子向け1000mの結果", "村上"], ["男子向け1000mの結果", "松野"],
    ["女子向け1000mのタイム", "増岡"], ["男子向け1000mのタイム", "田上"],
    ["女子向け1000mの参加者", "山﨑"], ["男子向け1000mの参加者", "山本"],
    ["9月22日の女子向け1000mの結果", "角田"], ["9月22日の男子向け1000mの結果", "中尾"],
    ["女子向け1000mはどうだった？", "塚原"], ["男子向け1000mはどうだった？", "南本"],
  ])("routes intended-audience wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子主体1000mの結果", "村上"], ["男子主体1000mの結果", "松野"],
    ["女子主体1000mのタイム", "増岡"], ["男子主体1000mのタイム", "田上"],
    ["女子主体1000mの参加者", "山﨑"], ["男子主体1000mの参加者", "山本"],
    ["9月22日の女子主体1000mの結果", "角田"], ["9月22日の男子主体1000mの結果", "中尾"],
    ["女子主体1000mはどうだった？", "塚原"], ["男子主体1000mはどうだった？", "南本"],
  ])("routes subject-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子中心1000mの結果", "村上"], ["男子中心1000mの結果", "松野"],
    ["女子中心1000mのタイム", "増岡"], ["男子中心1000mのタイム", "田上"],
    ["女子中心1000mの参加者", "山﨑"], ["男子中心1000mの参加者", "山本"],
    ["9月22日の女子中心1000mの結果", "角田"], ["9月22日の男子中心1000mの結果", "中尾"],
    ["女子中心1000mはどうだった？", "塚原"], ["男子中心1000mはどうだった？", "南本"],
  ])("routes focus-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子側1000mの結果", "村上"], ["男子側1000mの結果", "松野"],
    ["女子側1000mのタイム", "増岡"], ["男子側1000mのタイム", "田上"],
    ["女子側1000mの参加者", "山﨑"], ["男子側1000mの参加者", "山本"],
    ["9月22日の女子側1000mの結果", "角田"], ["9月22日の男子側1000mの結果", "中尾"],
    ["女子側1000mはどうだった？", "塚原"], ["男子側1000mはどうだった？", "南本"],
  ])("routes gender-side wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子組別1000mの結果", "村上"], ["男子組別1000mの結果", "松野"],
    ["女子組別1000mのタイム", "増岡"], ["男子組別1000mのタイム", "田上"],
    ["女子組別1000mの参加者", "山﨑"], ["男子組別1000mの参加者", "山本"],
    ["9月22日の女子組別1000mの結果", "角田"], ["9月22日の男子組別1000mの結果", "中尾"],
    ["女子組別1000mはどうだった？", "塚原"], ["男子組別1000mはどうだった？", "南本"],
  ])("routes heat-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子班1000mの結果", "村上"], ["男子班1000mの結果", "松野"],
    ["女子班1000mのタイム", "増岡"], ["男子班1000mのタイム", "田上"],
    ["女子班1000mの参加者", "山﨑"], ["男子班1000mの参加者", "山本"],
    ["9月22日の女子班1000mの結果", "角田"], ["9月22日の男子班1000mの結果", "中尾"],
    ["女子班1000mはどうだった？", "塚原"], ["男子班1000mはどうだった？", "南本"],
  ])("routes group-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  ...
  it.each([
    ["女子区1000mの結果", "村上"], ["男子区1000mの結果", "松野"],
    ["女子区1000mのタイム", "増岡"], ["男子区1000mのタイム", "田上"],
    ["女子区1000mの参加者", "山﨑"], ["男子区1000mの参加者", "山本"],
    ["9月22日の女子区1000mの結果", "角田"], ["9月22日の男子区1000mの結果", "中尾"],
    ["女子区1000mはどうだった？", "塚原"], ["男子区1000mはどうだった？", "南本"],
  ])("routes district-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子部門別1000mの結果", "村上"], ["男子部門別1000mの結果", "松野"],
    ["女子部門別1000mのタイム", "増岡"], ["男子部門別1000mのタイム", "田上"],
    ["女子部門別1000mの参加者", "山﨑"], ["男子部門別1000mの参加者", "山本"],
    ["9月22日の女子部門別1000mの結果", "角田"], ["9月22日の男子部門別1000mの結果", "中尾"],
    ["女子部門別1000mはどうだった？", "塚原"], ["男子部門別1000mはどうだった？", "南本"],
  ])("routes division-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子枠別1000mの結果", "村上"], ["男子枠別1000mの結果", "松野"],
    ["女子枠別1000mのタイム", "増岡"], ["男子枠別1000mのタイム", "田上"],
    ["女子枠別1000mの参加者", "山﨑"], ["男子枠別1000mの参加者", "山本"],
    ["9月22日の女子枠別1000mの結果", "角田"], ["9月22日の男子枠別1000mの結果", "中尾"],
    ["女子枠別1000mはどうだった？", "塚原"], ["男子枠別1000mはどうだった？", "南本"],
  ])("routes slot-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子対象1000mの結果", "村上"], ["男子対象1000mの結果", "松野"],
    ["女子対象1000mのタイム", "増岡"], ["男子対象1000mのタイム", "田上"],
    ["女子対象1000mの参加者", "山﨑"], ["男子対象1000mの参加者", "山本"],
    ["9月22日の女子対象1000mの結果", "角田"], ["9月22日の男子対象1000mの結果", "中尾"],
    ["女子対象1000mはどうだった？", "塚原"], ["男子対象1000mはどうだった？", "南本"],
  ])("routes target-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子該当1000mの結果", "村上"], ["男子該当1000mの結果", "松野"],
    ["女子該当1000mのタイム", "増岡"], ["男子該当1000mのタイム", "田上"],
    ["女子該当1000mの参加者", "山﨑"], ["男子該当1000mの参加者", "山本"],
    ["9月22日の女子該当1000mの結果", "角田"], ["9月22日の男子該当1000mの結果", "中尾"],
    ["女子該当1000mはどうだった？", "塚原"], ["男子該当1000mはどうだった？", "南本"],
  ])("routes matching-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子対象者1000mの結果", "村上"], ["男子対象者1000mの結果", "松野"],
    ["女子対象者1000mのタイム", "増岡"], ["男子対象者1000mのタイム", "田上"],
    ["女子対象者1000mの参加者", "山﨑"], ["男子対象者1000mの参加者", "山本"],
    ["9月22日の女子対象者1000mの結果", "角田"], ["9月22日の男子対象者1000mの結果", "中尾"],
    ["女子対象者1000mはどうだった？", "塚原"], ["男子対象者1000mはどうだった？", "南本"],
  ])("routes target-person wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子参加者1000mの結果", "村上"], ["男子参加者1000mの結果", "松野"],
    ["女子参加者1000mのタイム", "増岡"], ["男子参加者1000mのタイム", "田上"],
    ["女子参加者1000mの参加者", "山﨑"], ["男子参加者1000mの参加者", "山本"],
    ["9月22日の女子参加者1000mの結果", "角田"], ["9月22日の男子参加者1000mの結果", "中尾"],
    ["女子参加者1000mはどうだった？", "塚原"], ["男子参加者1000mはどうだった？", "南本"],
  ])("routes participant-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子走者1000mの結果", "村上"], ["男子走者1000mの結果", "松野"],
    ["女子走者1000mのタイム", "増岡"], ["男子走者1000mのタイム", "田上"],
    ["女子走者1000mの参加者", "山﨑"], ["男子走者1000mの参加者", "山本"],
    ["9月22日の女子走者1000mの結果", "角田"], ["9月22日の男子走者1000mの結果", "中尾"],
    ["女子走者1000mはどうだった？", "塚原"], ["男子走者1000mはどうだった？", "南本"],
  ])("routes runner-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子ランナー1000mの結果", "村上"], ["男子ランナー1000mの結果", "松野"],
    ["女子ランナー1000mのタイム", "増岡"], ["男子ランナー1000mのタイム", "田上"],
    ["女子ランナー1000mの参加者", "山﨑"], ["男子ランナー1000mの参加者", "山本"],
    ["9月22日の女子ランナー1000mの結果", "角田"], ["9月22日の男子ランナー1000mの結果", "中尾"],
    ["女子ランナー1000mはどうだった？", "塚原"], ["男子ランナー1000mはどうだった？", "南本"],
  ])("routes runner-term wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子選手団1000mの結果", "村上"], ["男子選手団1000mの結果", "松野"],
    ["女子選手団1000mのタイム", "増岡"], ["男子選手団1000mのタイム", "田上"],
    ["女子選手団1000mの参加者", "山﨑"], ["男子選手団1000mの参加者", "山本"],
    ["9月22日の女子選手団1000mの結果", "角田"], ["9月22日の男子選手団1000mの結果", "中尾"],
    ["女子選手団1000mはどうだった？", "塚原"], ["男子選手団1000mはどうだった？", "南本"],
  ])("routes squad-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子陣1000mの結果", "村上"], ["男子陣1000mの結果", "松野"],
    ["女子陣1000mのタイム", "増岡"], ["男子陣1000mのタイム", "田上"],
    ["女子陣1000mの参加者", "山﨑"], ["男子陣1000mの参加者", "山本"],
    ["9月22日の女子陣1000mの結果", "角田"], ["9月22日の男子陣1000mの結果", "中尾"],
    ["女子陣1000mはどうだった？", "塚原"], ["男子陣1000mはどうだった？", "南本"],
  ])("routes side-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子枠内1000mの結果", "村上"], ["男子枠内1000mの結果", "松野"],
    ["女子枠内1000mのタイム", "増岡"], ["男子枠内1000mのタイム", "田上"],
    ["女子枠内1000mの参加者", "山﨑"], ["男子枠内1000mの参加者", "山本"],
    ["9月22日の女子枠内1000mの結果", "角田"], ["9月22日の男子枠内1000mの結果", "中尾"],
    ["女子枠内1000mはどうだった？", "塚原"], ["男子枠内1000mはどうだった？", "南本"],
  ])("routes slot-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子限定1000mの結果", "村上"], ["男子限定1000mの結果", "松野"],
    ["女子限定1000mのタイム", "増岡"], ["男子限定1000mのタイム", "田上"],
    ["女子限定1000mの参加者", "山﨑"], ["男子限定1000mの参加者", "山本"],
    ["9月22日の女子限定1000mの結果", "角田"], ["9月22日の男子限定1000mの結果", "中尾"],
    ["女子限定1000mはどうだった？", "塚原"], ["男子限定1000mはどうだった？", "南本"],
  ])("routes limited-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子専用1000mの結果", "村上"], ["男子専用1000mの結果", "松野"],
    ["女子専用1000mのタイム", "増岡"], ["男子専用1000mのタイム", "田上"],
    ["女子専用1000mの参加者", "山﨑"], ["男子専用1000mの参加者", "山本"],
    ["9月22日の女子専用1000mの結果", "角田"], ["9月22日の男子専用1000mの結果", "中尾"],
    ["女子専用1000mはどうだった？", "塚原"], ["男子専用1000mはどうだった？", "南本"],
  ])("routes dedicated-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子別1000mの結果", "村上"], ["男子別1000mの結果", "松野"],
    ["女子別1000mのタイム", "増岡"], ["男子別1000mのタイム", "田上"],
    ["女子別1000mの参加者", "山﨑"], ["男子別1000mの参加者", "山本"],
    ["9月22日の女子別1000mの結果", "角田"], ["9月22日の男子別1000mの結果", "中尾"],
    ["女子別1000mはどうだった？", "塚原"], ["男子別1000mはどうだった？", "南本"],
  ])("routes gender-separated wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子別枠1000mの結果", "村上"], ["男子別枠1000mの結果", "松野"],
    ["女子別枠1000mのタイム", "増岡"], ["男子別枠1000mのタイム", "田上"],
    ["女子別枠1000mの参加者", "山﨑"], ["男子別枠1000mの参加者", "山本"],
    ["9月22日の女子別枠1000mの結果", "角田"], ["9月22日の男子別枠1000mの結果", "中尾"],
    ["女子別枠1000mはどうだった？", "塚原"], ["男子別枠1000mはどうだった？", "南本"],
  ])("routes separate-slot wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子専攻1000mの結果", "村上"], ["男子専攻1000mの結果", "松野"],
    ["女子専攻1000mのタイム", "増岡"], ["男子専攻1000mのタイム", "田上"],
    ["女子専攻1000mの参加者", "山﨑"], ["男子専攻1000mの参加者", "山本"],
    ["9月22日の女子専攻1000mの結果", "角田"], ["9月22日の男子専攻1000mの結果", "中尾"],
    ["女子専攻1000mはどうだった？", "塚原"], ["男子専攻1000mはどうだった？", "南本"],
  ])("routes specialization wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子系1000mの結果", "村上"], ["男子系1000mの結果", "松野"],
    ["女子系1000mのタイム", "増岡"], ["男子系1000mのタイム", "田上"],
    ["女子系1000mの参加者", "山﨑"], ["男子系1000mの参加者", "山本"],
    ["9月22日の女子系1000mの結果", "角田"], ["9月22日の男子系1000mの結果", "中尾"],
    ["女子系1000mはどうだった？", "塚原"], ["男子系1000mはどうだった？", "南本"],
  ])("routes gender-style wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子寄り1000mの結果", "村上"], ["男子寄り1000mの結果", "松野"],
    ["女子寄り1000mのタイム", "増岡"], ["男子寄り1000mのタイム", "田上"],
    ["女子寄り1000mの参加者", "山﨑"], ["男子寄り1000mの参加者", "山本"],
    ["9月22日の女子寄り1000mの結果", "角田"], ["9月22日の男子寄り1000mの結果", "中尾"],
    ["女子寄り1000mはどうだった？", "塚原"], ["男子寄り1000mはどうだった？", "南本"],
  ])("routes gender-leaning wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子優先1000mの結果", "村上"], ["男子優先1000mの結果", "松野"],
    ["女子優先1000mのタイム", "増岡"], ["男子優先1000mのタイム", "田上"],
    ["女子優先1000mの参加者", "山﨑"], ["男子優先1000mの参加者", "山本"],
    ["9月22日の女子優先1000mの結果", "角田"], ["9月22日の男子優先1000mの結果", "中尾"],
    ["女子優先1000mはどうだった？", "塚原"], ["男子優先1000mはどうだった？", "南本"],
  ])("routes priority-group wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子区分内1000mの結果", "村上"], ["男子区分内1000mの結果", "松野"],
    ["女子区分内1000mのタイム", "増岡"], ["男子区分内1000mのタイム", "田上"],
    ["女子区分内1000mの参加者", "山﨑"], ["男子区分内1000mの参加者", "山本"],
    ["9月22日の女子区分内1000mの結果", "角田"], ["9月22日の男子区分内1000mの結果", "中尾"],
    ["女子区分内1000mはどうだった？", "塚原"], ["男子区分内1000mはどうだった？", "南本"],
  ])("routes segment-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子編成1000mの結果", "村上"], ["男子編成1000mの結果", "松野"],
    ["女子編成1000mのタイム", "増岡"], ["男子編成1000mのタイム", "田上"],
    ["女子編成1000mの参加者", "山﨑"], ["男子編成1000mの参加者", "山本"],
    ["9月22日の女子編成1000mの結果", "角田"], ["9月22日の男子編成1000mの結果", "中尾"],
    ["女子編成1000mはどうだった？", "塚原"], ["男子編成1000mはどうだった？", "南本"],
  ])("routes composition-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子構成1000mの結果", "村上"], ["男子構成1000mの結果", "松野"],
    ["女子構成1000mのタイム", "増岡"], ["男子構成1000mのタイム", "田上"],
    ["女子構成1000mの参加者", "山﨑"], ["男子構成1000mの参加者", "山本"],
    ["9月22日の女子構成1000mの結果", "角田"], ["9月22日の男子構成1000mの結果", "中尾"],
    ["女子構成1000mはどうだった？", "塚原"], ["男子構成1000mはどうだった？", "南本"],
  ])("routes structure-label wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子種別ごと1000mの結果", "村上"], ["男子種別ごと1000mの結果", "松野"],
    ["女子種別ごと1000mのタイム", "増岡"], ["男子種別ごと1000mのタイム", "田上"],
    ["女子種別ごと1000mの参加者", "山﨑"], ["男子種別ごと1000mの参加者", "山本"],
    ["9月22日の女子種別ごと1000mの結果", "角田"], ["9月22日の男子種別ごと1000mの結果", "中尾"],
    ["女子種別ごと1000mはどうだった？", "塚原"], ["男子種別ごと1000mはどうだった？", "南本"],
  ])("routes type-wise wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子カテゴリー内1000mの結果", "村上"], ["男子カテゴリー内1000mの結果", "松野"],
    ["女子カテゴリー内1000mのタイム", "増岡"], ["男子カテゴリー内1000mのタイム", "田上"],
    ["女子カテゴリー内1000mの参加者", "山﨑"], ["男子カテゴリー内1000mの参加者", "山本"],
    ["9月22日の女子カテゴリー内1000mの結果", "角田"], ["9月22日の男子カテゴリー内1000mの結果", "中尾"],
    ["女子カテゴリー内1000mはどうだった？", "塚原"], ["男子カテゴリー内1000mはどうだった？", "南本"],
  ])("routes category-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子部門内1000mの結果", "村上"], ["男子部門内1000mの結果", "松野"],
    ["女子部門内1000mのタイム", "増岡"], ["男子部門内1000mのタイム", "田上"],
    ["女子部門内1000mの参加者", "山﨑"], ["男子部門内1000mの参加者", "山本"],
    ["9月22日の女子部門内1000mの結果", "角田"], ["9月22日の男子部門内1000mの結果", "中尾"],
    ["女子部門内1000mはどうだった？", "塚原"], ["男子部門内1000mはどうだった？", "南本"],
  ])("routes division-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子クラス内1000mの結果", "村上"], ["男子クラス内1000mの結果", "松野"],
    ["女子クラス内1000mのタイム", "増岡"], ["男子クラス内1000mのタイム", "田上"],
    ["女子クラス内1000mの参加者", "山﨑"], ["男子クラス内1000mの参加者", "山本"],
    ["9月22日の女子クラス内1000mの結果", "角田"], ["9月22日の男子クラス内1000mの結果", "中尾"],
    ["女子クラス内1000mはどうだった？", "塚原"], ["男子クラス内1000mはどうだった？", "南本"],
  ])("routes class-scope wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子学年別1000mの結果", "村上"], ["男子学年別1000mの結果", "松野"],
    ["女子学年別1000mのタイム", "増岡"], ["男子学年別1000mのタイム", "田上"],
    ["女子学年別1000mの参加者", "山﨑"], ["男子学年別1000mの参加者", "山本"],
    ["9月22日の女子学年別1000mの結果", "角田"], ["9月22日の男子学年別1000mの結果", "中尾"],
    ["女子学年別1000mはどうだった？", "塚原"], ["男子学年別1000mはどうだった？", "南本"],
  ])("routes grade-detail wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子選抜枠1000mの結果", "村上"], ["男子選抜枠1000mの結果", "松野"],
    ["女子選抜枠1000mのタイム", "増岡"], ["男子選抜枠1000mのタイム", "田上"],
    ["女子選抜枠1000mの参加者", "山﨑"], ["男子選抜枠1000mの参加者", "山本"],
    ["9月22日の女子選抜枠1000mの結果", "角田"], ["9月22日の男子選抜枠1000mの結果", "中尾"],
    ["女子選抜枠1000mはどうだった？", "塚原"], ["男子選抜枠1000mはどうだった？", "南本"],
  ])("routes selection-slot wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子候補枠1000mの結果", "村上"], ["男子候補枠1000mの結果", "松野"],
    ["女子候補枠1000mのタイム", "増岡"], ["男子候補枠1000mのタイム", "田上"],
    ["女子候補枠1000mの参加者", "山﨑"], ["男子候補枠1000mの参加者", "山本"],
    ["9月22日の女子候補枠1000mの結果", "角田"], ["9月22日の男子候補枠1000mの結果", "中尾"],
    ["女子候補枠1000mはどうだった？", "塚原"], ["男子候補枠1000mはどうだった？", "南本"],
  ])("routes candidate-slot wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の部1000mの結果", "村上"], ["男子の部1000mの結果", "松野"],
    ["女子の部1000mのタイム", "増岡"], ["男子の部1000mのタイム", "田上"],
    ["女子の部1000mの参加者", "山﨑"], ["男子の部1000mの参加者", "山本"],
    ["9月22日の女子の部1000mの結果", "角田"], ["9月22日の男子の部1000mの結果", "中尾"],
    ["女子の部1000mはどうだった？", "塚原"], ["男子の部1000mはどうだった？", "南本"],
  ])("routes gender-division wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のカテゴリー1000mの結果", "村上"], ["男子のカテゴリー1000mの結果", "松野"],
    ["女子のカテゴリー1000mのタイム", "増岡"], ["男子のカテゴリー1000mのタイム", "田上"],
    ["女子のカテゴリー1000mの参加者", "山﨑"], ["男子のカテゴリー1000mの参加者", "山本"],
    ["9月22日の女子のカテゴリー1000mの結果", "角田"], ["9月22日の男子のカテゴリー1000mの結果", "中尾"],
    ["女子のカテゴリー1000mはどうだった？", "塚原"], ["男子のカテゴリー1000mはどうだった？", "南本"],
  ])("routes category-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の区分1000mの結果", "村上"], ["男子の区分1000mの結果", "松野"],
    ["女子の区分1000mのタイム", "増岡"], ["男子の区分1000mのタイム", "田上"],
    ["女子の区分1000mの参加者", "山﨑"], ["男子の区分1000mの参加者", "山本"],
    ["9月22日の女子の区分1000mの結果", "角田"], ["9月22日の男子の区分1000mの結果", "中尾"],
    ["女子の区分1000mはどうだった？", "塚原"], ["男子の区分1000mはどうだった？", "南本"],
  ])("routes segment-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のクラス1000mの結果", "村上"], ["男子のクラス1000mの結果", "松野"],
    ["女子のクラス1000mのタイム", "増岡"], ["男子のクラス1000mのタイム", "田上"],
    ["女子のクラス1000mの参加者", "山﨑"], ["男子のクラス1000mの参加者", "山本"],
    ["9月22日の女子のクラス1000mの結果", "角田"], ["9月22日の男子のクラス1000mの結果", "中尾"],
    ["女子のクラス1000mはどうだった？", "塚原"], ["男子のクラス1000mはどうだった？", "南本"],
  ])("routes class-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の学年1000mの結果", "村上"], ["男子の学年1000mの結果", "松野"],
    ["女子の学年1000mのタイム", "増岡"], ["男子の学年1000mのタイム", "田上"],
    ["女子の学年1000mの参加者", "山﨑"], ["男子の学年1000mの参加者", "山本"],
    ["9月22日の女子の学年1000mの結果", "角田"], ["9月22日の男子の学年1000mの結果", "中尾"],
    ["女子の学年1000mはどうだった？", "塚原"], ["男子の学年1000mはどうだった？", "南本"],
  ])("routes grade-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の種別1000mの結果", "村上"], ["男子の種別1000mの結果", "松野"],
    ["女子の種別1000mのタイム", "増岡"], ["男子の種別1000mのタイム", "田上"],
    ["女子の種別1000mの参加者", "山﨑"], ["男子の種別1000mの参加者", "山本"],
    ["9月22日の女子の種別1000mの結果", "角田"], ["9月22日の男子の種別1000mの結果", "中尾"],
    ["女子の種別1000mはどうだった？", "塚原"], ["男子の種別1000mはどうだった？", "南本"],
  ])("routes type-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の部門1000mの結果", "村上"], ["男子の部門1000mの結果", "松野"],
    ["女子の部門1000mのタイム", "増岡"], ["男子の部門1000mのタイム", "田上"],
    ["女子の部門1000mの参加者", "山﨑"], ["男子の部門1000mの参加者", "山本"],
    ["9月22日の女子の部門1000mの結果", "角田"], ["9月22日の男子の部門1000mの結果", "中尾"],
    ["女子の部門1000mはどうだった？", "塚原"], ["男子の部門1000mはどうだった？", "南本"],
  ])("routes division-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の枠1000mの結果", "村上"], ["男子の枠1000mの結果", "松野"],
    ["女子の枠1000mのタイム", "増岡"], ["男子の枠1000mのタイム", "田上"],
    ["女子の枠1000mの参加者", "山﨑"], ["男子の枠1000mの参加者", "山本"],
    ["9月22日の女子の枠1000mの結果", "角田"], ["9月22日の男子の枠1000mの結果", "中尾"],
    ["女子の枠1000mはどうだった？", "塚原"], ["男子の枠1000mはどうだった？", "南本"],
  ])("routes slot-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のグループ1000mの結果", "村上"], ["男子のグループ1000mの結果", "松野"],
    ["女子のグループ1000mのタイム", "増岡"], ["男子のグループ1000mのタイム", "田上"],
    ["女子のグループ1000mの参加者", "山﨑"], ["男子のグループ1000mの参加者", "山本"],
    ["9月22日の女子のグループ1000mの結果", "角田"], ["9月22日の男子のグループ1000mの結果", "中尾"],
    ["女子のグループ1000mはどうだった？", "塚原"], ["男子のグループ1000mはどうだった？", "南本"],
  ])("routes group-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のチーム1000mの結果", "村上"], ["男子のチーム1000mの結果", "松野"],
    ["女子のチーム1000mのタイム", "増岡"], ["男子のチーム1000mのタイム", "田上"],
    ["女子のチーム1000mの参加者", "山﨑"], ["男子のチーム1000mの参加者", "山本"],
    ["9月22日の女子のチーム1000mの結果", "角田"], ["9月22日の男子のチーム1000mの結果", "中尾"],
    ["女子のチーム1000mはどうだった？", "塚原"], ["男子のチーム1000mはどうだった？", "南本"],
  ])("routes team-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の選抜1000mの結果", "村上"], ["男子の選抜1000mの結果", "松野"],
    ["女子の選抜1000mのタイム", "増岡"], ["男子の選抜1000mのタイム", "田上"],
    ["女子の選抜1000mの参加者", "山﨑"], ["男子の選抜1000mの参加者", "山本"],
    ["9月22日の女子の選抜1000mの結果", "角田"], ["9月22日の男子の選抜1000mの結果", "中尾"],
    ["女子の選抜1000mはどうだった？", "塚原"], ["男子の選抜1000mはどうだった？", "南本"],
  ])("routes selection-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の候補1000mの結果", "村上"], ["男子の候補1000mの結果", "松野"],
    ["女子の候補1000mのタイム", "増岡"], ["男子の候補1000mのタイム", "田上"],
    ["女子の候補1000mの参加者", "山﨑"], ["男子の候補1000mの参加者", "山本"],
    ["9月22日の女子の候補1000mの結果", "角田"], ["9月22日の男子の候補1000mの結果", "中尾"],
    ["女子の候補1000mはどうだった？", "塚原"], ["男子の候補1000mはどうだった？", "南本"],
  ])("routes candidate-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の登録1000mの結果", "村上"], ["男子の登録1000mの結果", "松野"],
    ["女子の登録1000mのタイム", "増岡"], ["男子の登録1000mのタイム", "田上"],
    ["女子の登録1000mの参加者", "山﨑"], ["男子の登録1000mの参加者", "山本"],
    ["9月22日の女子の登録1000mの結果", "角田"], ["9月22日の男子の登録1000mの結果", "中尾"],
    ["女子の登録1000mはどうだった？", "塚原"], ["男子の登録1000mはどうだった？", "南本"],
  ])("routes registration-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の所属1000mの結果", "村上"], ["男子の所属1000mの結果", "松野"],
    ["女子の所属1000mのタイム", "増岡"], ["男子の所属1000mのタイム", "田上"],
    ["女子の所属1000mの参加者", "山﨑"], ["男子の所属1000mの参加者", "山本"],
    ["9月22日の女子の所属1000mの結果", "角田"], ["9月22日の男子の所属1000mの結果", "中尾"],
    ["女子の所属1000mはどうだった？", "塚原"], ["男子の所属1000mはどうだった？", "南本"],
  ])("routes affiliation-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の代表1000mの結果", "村上"], ["男子の代表1000mの結果", "松野"],
    ["女子の代表1000mのタイム", "増岡"], ["男子の代表1000mのタイム", "田上"],
    ["女子の代表1000mの参加者", "山﨑"], ["男子の代表1000mの参加者", "山本"],
    ["9月22日の女子の代表1000mの結果", "角田"], ["9月22日の男子の代表1000mの結果", "中尾"],
    ["女子の代表1000mはどうだった？", "塚原"], ["男子の代表1000mはどうだった？", "南本"],
  ])("routes representative-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子のレース1000mの結果", "村上"], ["男子のレース1000mの結果", "松野"],
    ["女子のレース1000mのタイム", "増岡"], ["男子のレース1000mのタイム", "田上"],
    ["女子のレース1000mの参加者", "山﨑"], ["男子のレース1000mの参加者", "山本"],
    ["9月22日の女子のレース1000mの結果", "角田"], ["9月22日の男子のレース1000mの結果", "中尾"],
    ["女子のレース1000mはどうだった？", "塚原"], ["男子のレース1000mはどうだった？", "南本"],
  ])("routes race-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の大会1000mの結果", "村上"], ["男子の大会1000mの結果", "松野"],
    ["女子の大会1000mのタイム", "増岡"], ["男子の大会1000mのタイム", "田上"],
    ["女子の大会1000mの参加者", "山﨑"], ["男子の大会1000mの参加者", "山本"],
    ["9月22日の女子の大会1000mの結果", "角田"], ["9月22日の男子の大会1000mの結果", "中尾"],
    ["女子の大会1000mはどうだった？", "塚原"], ["男子の大会1000mはどうだった？", "南本"],
  ])("routes meet-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の記録会1000mの結果", "村上"], ["男子の記録会1000mの結果", "松野"],
    ["女子の記録会1000mのタイム", "増岡"], ["男子の記録会1000mのタイム", "田上"],
    ["女子の記録会1000mの参加者", "山﨑"], ["男子の記録会1000mの参加者", "山本"],
    ["9月22日の女子の記録会1000mの結果", "角田"], ["9月22日の男子の記録会1000mの結果", "中尾"],
    ["女子の記録会1000mはどうだった？", "塚原"], ["男子の記録会1000mはどうだった？", "南本"],
  ])("routes record-meet-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の記録1000mの結果", "村上"], ["男子の記録1000mの結果", "松野"],
    ["女子の記録1000mのタイム", "増岡"], ["男子の記録1000mのタイム", "田上"],
    ["女子の記録1000mの参加者", "山﨑"], ["男子の記録1000mの参加者", "山本"],
    ["9月22日の女子の記録1000mの結果", "角田"], ["9月22日の男子の記録1000mの結果", "中尾"],
    ["女子の記録1000mはどうだった？", "塚原"], ["男子の記録1000mはどうだった？", "南本"],
  ])("routes record-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の結果1000mの結果", "村上"], ["男子の結果1000mの結果", "松野"],
    ["女子の結果1000mのタイム", "増岡"], ["男子の結果1000mのタイム", "田上"],
    ["女子の結果1000mの参加者", "山﨑"], ["男子の結果1000mの参加者", "山本"],
    ["9月22日の女子の結果1000mの結果", "角田"], ["9月22日の男子の結果1000mの結果", "中尾"],
    ["女子の結果1000mはどうだった？", "塚原"], ["男子の結果1000mはどうだった？", "南本"],
  ])("routes result-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女子の子ども1000mの結果", "村上"], ["男子の子ども1000mの結果", "松野"],
    ["女子の子ども1000mのタイム", "増岡"], ["男子の子ども1000mのタイム", "田上"],
    ["女子の子ども1000mの参加者", "山﨑"], ["男子の子ども1000mの参加者", "山本"],
    ["9月22日の女子の子ども1000mの結果", "角田"], ["9月22日の男子の子ども1000mの結果", "中尾"],
    ["女子の子ども1000mはどうだった？", "塚原"], ["男子の子ども1000mはどうだった？", "南本"],
  ])("routes child-particle wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
  });
  ...
  it.each([
    ["女児1000mの結果", "村上"], ["男児1000mの結果", "松野"],
    ["女児1000mのタイム", "増岡"], ["男児1000mのタイム", "田上"],
    ["女児1000mの参加者", "山﨑"], ["男児1000mの参加者", "山本"],
    ["9月22日の女児1000mの結果", "角田"], ["9月22日の男児1000mの結果", "中尾"],
    ["女児1000mはどうだった？", "塚原"], ["男児1000mはどうだった？", "南本"],
  ])("routes child-gender wording: %s", async (question, expected) => {
    const result = await ask(question);
    expect(result.sources).toEqual(["drive-text/練習/玉名市練習会/2026-09-22.md"]);
    expect(result.text).toContain(expected);
