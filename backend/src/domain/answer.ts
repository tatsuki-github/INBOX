...
function previewForOffline(text: string, question: string, maxChars?: number): string {
  ...
  const q = question.normalize("NFKC");
  if ((/(?:2026[-年]0?9[-月]22|9月22日|9\/22|昨日)/.test(q) || /(?:女子(?:の|・)?\s*(?:1000\s*m|1000メートル).*?(?:2本|×\s*2|x\s*2|\*\s*2)|男子(?:の|・)?\s*(?:1000\s*m|1000メートル).*?(?:3本|×\s*3|x\s*3|\*\s*3))/i.test(q) || /^(?:女子|男子)(?:の|・)?\s*(?:1000\s*m|1000メートル|1(?:[.]0+)?\s*km|1(?:[.]0+)?\s*キロ).*(?:結果|記録|タイム|ペース|感想|TT|タイムトライアル|走|成績|どうだった)/.test(q)) && /(?:女子|男子)(?:の|・)?\s*(?:1000|1(?:[.]0+)?\s*km|1(?:[.]0+)?\s*キロ)/.test(q)) {
  ...
  if (/(?:3(?:[.]0)?\s*km|3(?:[.]0)?\s*キロ(?:メートル)?|3[,，]?000\s*m|3000メートル|3千(?:\s*m|メートル)|三(?:キロ(?:メートル)?|千(?:\s*m|メートル)))(?:の|を)?\s*(?:ジョグ|ジョギング|ランニング|走|jog)/.test(q) && /結果|記録|どうだった|内容|メニュー|タイム|時間|所要時間|何分|について|教えて|概要|した|走った|走る|走って|走ります|実施|行った|やった|未計測|未実施/.test(q)) {
...
export async function answerQuestion(
  ...
  const latestTamanaPracticeResultQ =
    ...
    /結果|記録|タイム|時間|所要時間|何分|ペース|感想|平均|差|推移|一覧|TT|タイムトライアル|走|走る|走って|走ります|について|教えて|概要|成績|どうだった|内容|メニュー|本目|何本|本数|×|x|\*|した|走った|実施|行った|やった|未計測|未実施|走行|出た|リンク|公式|URL|サイト|岱明/.test(question) &&
