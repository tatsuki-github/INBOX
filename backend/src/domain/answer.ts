...
function previewForOffline(text: string, question: string, maxChars?: number): string {
  ...
  const q = question.normalize("NFKC");
  if ((/(?:2026[-年]0?9[-月]22|9月22日|9\/22|昨日)/.test(q) || /(?:女子(?:の|・)?\s*1000\s*m.*(?:2本|×\s*2|x\s*2)|男子(?:の|・)?\s*1000\s*m.*(?:3本|×\s*3|x\s*3))/i.test(q) || /^(?:女子|男子)(?:の|・)?\s*(?:1000\s*m|1000メートル|1\s*km|1\s*キロ).*(?:結果|記録|タイム|成績|どうだった)/.test(q)) && /(?:女子|男子)(?:の|・)?\s*(?:1000|1\s*km|1\s*キロ)/.test(q)) {
  ...
  if (/(?:3(?:[.]0)?\s*km|3(?:[.]0)?\s*キロ(?:メートル)?|3[,，]?000\s*m|3000メートル|3千(?:\s*m|メートル)|三(?:キロ(?:メートル)?|千(?:\s*m|メートル)))(?:の\s*)?\s*ジョグ/.test(q) && /結果|記録|どうだった|内容|メニュー|タイム|時間|所要時間|何分|について|教えて|概要|した|走った|実施|行った|やった|未計測|未実施/.test(q)) {
    return "玉名市練習会＆BBQ（2026-09-22）のメニューは、動きづくり、3kmジョグ、女子1000m×2本、男子1000m×3本です。3kmジョグの個別タイムは記録されていません。";
  }
  if (/^1000\s*(?:m|メートル)/.test(q) && /本目|結果|記録|タイム|メニュー|何本|本数|どうだった|×\s*[23]|x\s*[23]/i.test(q)) {
...
export async function answerQuestion(
  ...
  const latestTamanaPracticeResultQ =
    (/2026-09-22.*(?:練習会|(?:女子|男子)(?:の|・)?\s*1000)|練習会.*2026-09-22|2026-09-22.*岱明|(?:昨日|9月22日|9\/22).*?(?:女子|男子)(?:の|・)?\s*1000|(?:女子|男子)(?:の|・)?\s*1000.*(?:昨日|9月22日|9\/22)|2026年?9月22日.*(?:女子|男子)(?:の|・)?\s*1000|(?:女子(?:の|・)?\s*1000\s*m.*(?:2本|×\s*2|x\s*2)|男子(?:の|・)?\s*1000\s*m.*(?:3本|×\s*3|x\s*3))|^1000\s*(?:m|メートル).*(?:本目|結果|記録|タイム|メニュー|何本|本数|どうだった|×\s*[23]|x\s*[23])|(?:3(?:[.]0)?\s*km|3(?:[.]0)?\s*キロ(?:メートル)?|3[,，]?000\s*m|3000メートル|3千(?:\s*m|メートル)|三(?:キロ(?:メートル)?|千(?:\s*m|メートル)))(?:の\s*)?\s*ジョグ|動きづくり.*(?:結果|実施|どうだった)/i.test(question) ||
      /玉名市.*練習会|練習会.*玉名市|岱明.*練習会|練習会.*岱明|岱明.*(?:女子|男子)(?:の|・)?\s*(?:1000\s*m|1000メートル|1\s*km|1\s*キロ)|(?:女子|男子)(?:の|・)?\s*(?:1000\s*m|1000メートル|1\s*km|1\s*キロ).*岱明|^(?:女子|男子)(?:の|・)?\s*(?:1000\s*m|1000メートル|1\s*km|1\s*キロ)|(?:村上|増岡|山﨑|角田|塚原|柴尾|松野|田上|山本|中尾|松本|南本|嶋田|高田(?!麻那)).*(?:1000\s*m|1000メートル|1\s*km|1\s*キロ)|^練習会|昨日.*練習会|練習会.*昨日|昨日.*(?:女子|男子)1000|9月22日.*練習会|練習会.*9月22日|9\/22.*練習会|練習会.*9\/22/.test(question)) &&
    /結果|記録|タイム|時間|所要時間|何分|について|教えて|概要|成績|どうだった|内容|メニュー|本目|何本|本数|×|x|した|走った|実施|行った|やった|未計測|未実施|走行|出た|リンク|公式|URL|サイト|岱明/.test(question) &&
