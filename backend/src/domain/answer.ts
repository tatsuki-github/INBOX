...
function previewForOffline(text: string, question: string, maxChars?: number): string {
  ...
  if (!/男子|女子/.test(q) && /荒玉|駅伝/.test(q) && /(?:[6-9]|1[0-5])位|六位|七位|八位|九位/.test(q)) {
    const rank = q.match(/([6-9]|1[0-5])位/)?.[1] ?? (/六位/.test(q) ? "6" : /七位/.test(q) ? "7" : /八位/.test(q) ? "8" : /九位/.test(q) ? "9" : undefined);
...
function offlineAnswer(
  ...
  const winnerYearTeamLookup =
    ...
    const unqualifiedSixthPlaceLookup =
      /(?:[6-9]|1[0-5])位|六位|七位|八位|九位/.test(question) &&
      /荒玉|駅伝/.test(question) &&
      !/男子|女子|20\d{2}|過去|歴代|区間/.test(question);
