            ...
            for team in y.get("teams", []):
              for leg in team.get("legs", []):
                ...
                if name and sec is not None:
                    out.setdefault((year, gender, norm(name)), []).append({"team": team.get("team"), "leg": leg.get("leg"), "time": leg.get("split"), "sec": sec, "rank": leg.get("split_rank"), "team_rank": team.get("rank"), "total": team.get("total")})
    return out
...
def main() -> None:
    ...
    for athlete in current:
        for r in sb:
            ...
            for leg in legs:
                matches.append({"current": athlete["name"], "current_sb": athlete["sb"], "gender": athlete["gender"], "distance": athlete["distance"], "historical_year": r["year"], "historical_name": r["name"], "historical_school": r.get("所属"), "historical_sb": r["sb"], "sb_delta": delta, "team": leg["team"], "leg": leg["leg"], "ekiden_time": leg["time"], "ekiden_sec": leg["sec"], "split_rank": leg["rank"], "team_rank": leg["team_rank"], "team_total": leg["total"], "source": r["source"]})
    ...
    def render_table(gender: str) -> str:
        ...
        for m in gender_matches:
            rows.append("<tr>" + "".join(f"<td>{html.escape(str(v))}</td>" for v in [m['current'], fmt(m['current_sb']), m['distance'], m['historical_year'], m['historical_name'], m.get('historical_school') or '—', fmt(m['historical_sb']), f"{m['sb_delta']:+.2f}", m['team'], f"{m['leg']}区", m['ekiden_time'], f"{m['split_rank'] or '—'}", f"{m['team_rank']}位"]) + "</tr>")
        body = ''.join(rows) if rows else '<tr><td colspan="13">該当する駅伝実績はありません</td></tr>'
        return f'''<h2>{gender}</h2><div class="table-wrap"><table><thead><tr><th>今年度岱明</th><th>今年度SB</th><th>種目</th><th>過去年</th><th>過去選手（全校）</th><th>過去所属</th><th>過去SB</th><th>SB差</th><th>駅伝学校</th><th>駅伝区間</th><th>駅伝記録</th><th>区間順位</th><th>チーム順位</th></tr></thead><tbody>{body}</tbody></table></div>'''
