...
- 麻那／麻由の混同が減る（完全一致を優先）
- 地区ランキング・所属全記録がコーチ誘導ではなくコーパス抜粋で返す
- 類似質問（「一番速い」「トップ20」「全記録」「2位以内の回数」）も同じ経路
- 所属全記録で `out-analysis` 等の広い preferred プレフィックスを足さない（exact digest が埋もれない）
- 荒玉「過去の順位」は `aragyoku-teams/{校}.md` を exact pin
- トラック1周は `practice/daiming-practice-menus-kpace.md` を正本（LINE 会話に流さない）

## Alternatives Considered

- LLM に「同姓は別人」とだけ指示: 却下 — 検索段階で麻由が勝つと抜粋に載る
- SBデータベースのみでランキングをその場計算: 却下 — オフライン応答と再現性のためダイジェスト正本を採用
- preferred にディレクトリ hub を足して BM25 に任せる: 却下 — 兄弟 digest に埋もれる（玉南中→玉陵）
